...
ENT.Gauges = {
		...
		VSpeed = {
			Pos = Vector( 35, -1.5, -4.5 ),
			Ang = Angle(),
			Scale = 0.02
		};
		Clock = {
			Pos = Vector( 35, 15, -5.0 ),
			Ang = Angle(),
			Scale = 0.02
		};
		Altimeter = {
...
ENT.WingLiftMultiplier = 49	    -- tail lift coefficient
	...
	function ENT:PhysicsSimulate( phys, deltatime ) 
		...
		if( IsValid( self.Pilot ) && !self.Destroyed && self.CurVelocity ) then
			...
			if( self:GetVelocity():Length() > self.MaxVelocity / 4 ) then
				...
				self:GetPhysicsObject():AddAngleVelocity( Vector( 8.5, 0, -.25 ) * 1.0 * ( self.CurVelocity  / self.MaxVelocity ) ) 
