...
function SWEP:Deploy()
	...
	if( SERVER ) then 
		...
		self.Owner.Shield:SetModel("models/arleitiss/riotshield/shield.mdl")
		self.Owner.Shield:SetPos(self.Owner:LocalToWorld( Vector( 22	,-5, 10 ) ) ) -- self.Owner:GetPos() + Vector(10,0,15) + (self.Owner:GetForward()*25))
...
if( SERVER ) then 
	...
	function SWEP:Holster()
		...
		if( IsValid( self.Owner.Shield ) ) then 
			...
			self.Owner.Shield:SetPos( self.Owner:LocalToWorld( Vector( -29, -15, -4 ) ) )
			self.Owner.Shield:SetAngles( self.Owner:GetAngles() + Angle( 0,175,0 ) )
