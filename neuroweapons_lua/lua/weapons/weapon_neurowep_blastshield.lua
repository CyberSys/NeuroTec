...
function SWEP:Deploy()
	...
	if( SERVER ) then 
		...
		self.Owner.Shield:SetModel("models/bf4/bf4_shield.mdl")
		self.Owner.Shield:SetPos(self.Owner:LocalToWorld( Vector( 3,15, 14 ) ) ) -- self.Owner:GetPos() + Vector(10,0,15) + (self.Owner:GetForward()*25))
...
if( SERVER ) then 
	...
	function SWEP:Holster()
		...
		if( IsValid( self.Owner.Shield ) ) then 
			...
			self.Owner:SetEyeAngles( Angle( 0, a.y, a.r ) )
			self.Owner.Shield:SetParent( self.Owner )
			self.Owner.Shield:Fire("SetParentAttachmentMaintainOffset", "chest", 0.01)
			self.Owner.Shield:SetPos( self.Owner:LocalToWorld( Vector( -15, -30, 8 ) ) )
			self.Owner.Shield:SetAngles( self.Owner:GetAngles() + Angle( 0,170,0 ) )
