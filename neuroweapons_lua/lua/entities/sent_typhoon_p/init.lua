...
ENT.Model = "models/solcommand/typhoon/base.mdl"
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_typhoon_p" )
	ent:SetPos( SpawnPos )
...
function ENT:Think()
	...
	if ( self.IsDriving && IsValid( self.Pilot ) ) then
		...
		if( self.Pilot:KeyDown( IN_SPEED ) && self.LastRelease + 0.5 <= CurTime() ) then
...
function ENT:FireSTORM()
	...
	if ( IsValid( self.Missile[n] )) then
		...
		timer.Simple( 5, function() if(IsValid( self.Missile[n] ) ) then  self.Missile[n]:SetNoDraw(false) end end, self.Missile[n])
