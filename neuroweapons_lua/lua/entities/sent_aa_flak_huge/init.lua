...
function ENT:PhysicsCollide( data, physobj )
	...
	if ( data.Speed > 150 && data.DeltaTime > 0.1 ) then 
		...
		util.BlastDamage( self.Owner, self.Owner, data.HitPos, 2000, math.random( 500,1000 ))
		
		local fx = EffectData()
		fx:SetOrigin(data.HitPos)
		util.Effect("Flaksmoke_Big",fx)
...
function ENT:Think()
	...
	self:SetVelocity( self:GetForward() * 5500 )
	...
	if( self.AutoDetonateTimer >= self.MaxTime ) then
		...
		util.BlastDamage( self.Owner, self.Owner, self:GetPos() + p, 2000, math.random( 500,1000 ))
	...
	for k,v in pairs( ents.FindInSphere(self:GetPos(), 2000 ) ) do
		...
		if ( ( v:IsNPC() || v.HealthVal ) && v != self.Owner && v:GetParent() != self.Owner && self.AutoDetonateTimer > ( self.MaxTime * 0.5 ) ) then
			...
			util.BlastDamage( self.Owner, self.Owner, self:GetPos(), 2000, math.random( 500,1000 ))
