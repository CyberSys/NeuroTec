...
-- end
hook.Remove("SetupMove", "NeuroTec_SetupMove" )

hook.Add("Move", "NeuroTec_SetupMove", function( ply, mv ) 
		
	
	if( ply:IsBot() && IsValid( ply:GetScriptedVehicle() ) ) then 
	
		local ride = ply:GetScriptedVehicle() 
		if( ride.IsMicroCruiser ) then 
			...
			local a = ride:GetAngles()
			local pos = ride:GetPos() 
			local t = ply.Target 
			
			if( !IsValid( t ) ) then	
				
				local shortest = 12500 
				local dist = 0 
				local target = NULL 
				
				for k,v in ipairs( player.GetAll() ) do 
					
					if( v != ply && neuro_inLOS( v, ply ) ) then 
						dist = ( v:GetPos() - ply:GetPos() ):Length()
			...
			if( IsValid( t ) ) then 
				...
				if( !neuro_inLOS( ply, t ) ) then ply.Target = NULL return end 
				
				local tpos = t:GetPos() + t:GetVelocity() * FrameTime()
				local dist = ( ride:GetPos() - tpos ):Length() 
				-- print( t )
				local tride = NULL 
				if( t:IsPlayer() ) then 
					tride = t:GetScriptedVehicle()
					if( IsValid( tride ) ) then 
						
						if( tride:GetClass() == ride:GetClass() ) then ply.Target = NULL return end 
						
						tpos = tride:GetPos() + tride:GetVelocity() + Vector(0,0,16)
						-- print("what")
					end 
					
				end 
				-- if( !ply.LastGroupSwap ) then ply.LastGroupSwap = CurTime() end 
				
				if( ply.SelectedWeapon < ride.NumberOfWeaponGroups ) then 
					
					ply.SelectedWeapon = ply.SelectedWeapon + 1 
				...
				local targetAngle = ( tpos - pos ):GetNormalized():Angle()
				targetAngle:Normalize()
				ply.ClientVector = tpos 
				local ydiff = math.AngleDifference( a.y, targetAngle.y )
				
				local ft,ftrace = {},{}
				ft.start = ride:GetPos() 
				ft.endpos = ft.start + ride:GetForward() * 1000 + ride:GetRight() * math.sin(CurTime())*700
				ft.filter = { ride, ply, ride.Deck, ride.ShipParts }
				ft.mask = MASK_SOLID_BRUSHONLY
				ftrace = util.TraceLine( ft )
				-- ply:DrawLaserTracer( ft.start, ftrace.HitPos ) 
				
				-- if( ) then 
				
				if( !ftrace.Hit && math.abs( ydiff ) < 25   ) then 
					
					ride.Speed = ride.MaxVelocity
					ride.LastChadburn = CurTime() + 15 
				...
				local traceAng = ( ftrace.HitPos - ride:GetPos() ):Angle()
				traceAng:Normalize()
				
				local tdiff = math.AngleDifference( a.y, traceAng.y )
							
				local right = ydiff > 0 && ydiff < 180 
				local left = ydiff < 0  && ydiff > -180 
				local front = math.abs( ydiff ) > 110 
				local back = math.abs( ydiff ) < 60 
				
				if( !ftrace.Hit && dist > 1200 ) then 
					
					-- if( !ride.CanTurnStationary ) then 	
							
						ride.Speed = ride.MaxVelocity/3
						-- ride.LastChadburn = CurTime() + 15 
					...
					if( left && !front && !back ) then 
						
						ride.ShipAngleForceTargetValue = -0.0051 / ride.TurnModifierValue 
					
					elseif( right && !front && !back ) then 
					
						ride.ShipAngleForceTargetValue = 0.0051 / ride.TurnModifierValue 
					
					else
						
						ride.ShipAngleForceTargetValue = 0 
					
					end 
			
				end 
				
				if( ftrace.Hit && math.abs( tdiff ) > 0 ) then 
						
					ride.ShipAngleForceTargetValue = math.Clamp( math.abs(tdiff*10), -1, 1) * 0.0051 / ride.TurnModifierValue 
					
					
				end 
	
				-- print( ftrace.HitNormal ) 
				-- if( 
				...
				for k,v in pairs( ride.Weaponry ) do 
					
					local gunAng = v.Barrel:GetAngles()
					local gunTargetAng = ( tpos - v.Turret:GetPos()  ):GetNormalized():Angle()
					gunTargetAng:Normalize()
					local gunDiff = math.floor( math.abs( math.AngleDifference( gunAng.y, gunTargetAng.y ) ) )
					
					
					ply:SetEyeAngles( gunTargetAng )
					-- print( gunDiff )
					if( gunDiff <= 1 ) then 
						Count = Count + 1 
						
						
						v.IsReadyToFire = true 
						
						-- print( "FOYAAH", gunDiff )
						-- ply:DrawLaserTracer( v.Turret:GetPos(), v.Turret:GetPos() + gunTargetAng:Forward() * 5000  )
						-- ply:DrawLaserTracer( v.Turret:GetPos(), tpos )
						-- local btns = mv:GetButtons()
						-- btns = btns + IN_ATTACK 
						-- mv:SetButtons( IN_ATTACK ) 
						-- mv:SetButtons( IN_ATTACK )
						-- mv:SetOldButtons( IN_ATTACK )
						-- print( bit.bnot( mv:GetButtons(), IN_ATTACK )  )
					else
						
						v.IsReadyToFire = false 
...
function Meta:NeuroNaval_DefaultPhysSimulate( phys, deltatime )
	...
	if( self.ShipAngleForceCurrentValue && self.ShipAngleForceTargetValue ) then 
		...
		if( self:WaterLevel() > 0 && myang.r < 45 && myang.r > -45 && self.PropellerPos && !self.RudderIsFucked && !self.NoSideForce ) then 
			-- print("what")
		...
		if( self:WaterLevel() > 0 && !self.Destroyed ) then 
	...
	if ( self.IsBoating && self:WaterLevel() > 0 && !self.Destroyed ) then
		...
		if( IsValid( self.Pilot ) && !self.Pilot:IsBot() && self.LastChadburn != self.Speed && !self.Pilot:KeyDown( IN_FORWARD ) && !self.Pilot:KeyDown( IN_BACK )  && self:WaterLevel() != 3 ) then 
...
function Meta:NeuroNaval_DefaultCruiserThink()
	...
	if( self.ShipParts && self:SkinCount() > 0 ) then 
		
		for k,v in ipairs( self.ShipParts ) do 
			
			if IsValid( v ) && v:GetSkin() != self:GetSkin() then 
	...
	if( self.IsMicroSubmarine && self.HasDeck && !IsValid( self.Deck ) ) then 
		
		self.Destroyed = true 
			
		self.BuoyancyRatio = math.Approach( self.BuoyancyRatio, 0, 1 )
		
	end 
	
	if( self.ShipParts ) then 
		
		for k,v in pairs( self.ShipParts ) do 
			
			if( IsValid( v ) ) then 
				
				if( !IsValid( v.Weld ) ) then 
					
					self.BuoyancyRatio = self.BuoyancyRatio * .9
					constraint.RemoveAll( self )
					
				end 
				-- print( v.Weld )
				v:GetPhysicsObject():SetBuoyancyRatio( self.BuoyancyRatio )
			else
	...
	if ( self.IsBoating && IsValid( self.Pilot ) ) then
		...
		if( !self.LastHorn ) then self.LastHorn  = CurTime() end 
		
		if( self.Pilot:KeyDown( IN_ZOOM ) && self.LastHorn + 1.0 <= CurTime() && self:WaterLevel() != 3 ) then 
			
			local hornpitch = 100 
			if( self.HornPitch ) then 
				
				hornpitch = self.HornPitch
				
			end 
			
			self.LastHorn = CurTime() 
			self:EmitSound( "ships/horn.wav", 511, hornpitch )
		...
		if (  self.EquipmentNames && self.Pilot:KeyDown( IN_ATTACK2 ) && ( !self.WeaponSystems ) && self.LastAttackKeyDown + 0.5 < CurTime() ) then
	...
	if( !self.LastCrewUpdate ) then self.LastCrewUpdate = 0 end 
	
	if( self.LastCrewUpdate + 1.0 <= CurTime() ) then 
		
		self:NeuroNaval_CrewRepairFunction()
	...
	if( self.Weaponry && IsValid( self.Pilot )  && self.Pilot.SelectedWeapon ) then 
		...
		for k,v in pairs( self.Weaponry ) do 
			...
			if ( v.WeaponGroup == self.Pilot.SelectedWeapon && IsValid( v.Turret ) && !v.Destroyed ) then 
				...
				if( !self.Pilot:KeyDown( IN_ATTACK2 ) && !self.Pilot:IsBot() ) then 
				
					self.Pilot.ClientVector = tr.HitPos
					
				end 
				
				local hpos = self.Pilot.ClientVector or Vector()
		...
		if( self.Weaponry && self.LastAttackKeyDown && self.LastAttackKeyDown + .25 <= CurTime() ) then  
			...
			for k,v in pairs( self.Weaponry ) do 
				...
				if ( v.WeaponGroup == self.Pilot.SelectedWeapon && IsValid( v.Turret ) && !v.Destroyed && v.AmmoCounter > 0 ) then 
					...
					if ( v.AngDiff && v.AngDiff > -v.MaxYaw && v.AngDiff < v.MaxYaw && ( self.Pilot:KeyDown( IN_ATTACK ) || v.IsReadyToFire ) ) then 
...
function Meta:MicroShipGunFire( wep, idx, lastidx, targetpos, reps   )
	...
	if( reps > 0 ) then 
		...
		timer.Simple( wep.Cooldown, function() 
				
			if( IsValid( self ) ) then 
			
				self:MicroShipGunFire( wep, idx, lastidx, targetpos, reps-1 )
...
function Meta:YamatoFire( wep, idx, lastidx, targetpos  )
	...
	if( wep.UseHitScan ) then 
		...
		self:MicroShipGunFire( wep, idx, lastidx, targetpos, wep.NumShots or 0  )
...
function Meta:NeuroNaval_CrewRepairFunction()

	local _1pct = self.InitialHealth * .0005 
	
	if( !IsValid( self.Pilot ) ) then return end 
	
	if( self.EngineerHealth > 0 ) then 
		

		self.HealthVal = math.Approach( self.HealthVal, self.InitialHealth, _1pct * self.EngineerHealth/500 )
		
		if( IsValid( self.Deck ) ) then 
		
			self.Deck.HealthVal = math.Approach( self.Deck.HealthVal, self.InitialHealth, _1pct * self.EngineerHealth/500 )
	
		end 
		
		if( self.Ballasts ) then 
				
			local rnd = self.Ballasts[math.random(1,#self.Ballasts)]
			if( IsValid( rnd ) ) then 	
	...
	if( self.MedicHealth > 0 ) then 
		
		local inc = 1 * self.MedicHealth / 500 
		
		self.MedicHealth = math.Approach( self.MedicHealth, 500, inc ) 
		
		if( self.FiremenHealth > 0 ) then 
		
			self.FiremenHealth = math.Approach( self.FiremenHealth, 500,  inc ) 
		
		end 
		
		if( self.EngineerHealth > 0 ) then 
		
			self.EngineerHealth = math.Approach( self.EngineerHealth, 500,  inc  ) 
		
		end 
		
		self:NeuroNaval_UpdateCrewHealth()
	
	end 
	
	if( self.FiremenHealth > 0 ) then 
		
		local Chance = 15 - ( 15 * ( self.FiremenHealth / 500 ) )
		...
		if(math.random(0,Chance ) == 0 ) then 
			...
			if( self.Weaponry  ) then 
				...
				for k,v in ipairs( self.Weaponry ) do 
					...
					if( !v.Destroyed && v.HealthVal > 0 ) then 
						
						v.HealthVal = math.Approach( v.HealthVal, v.InitialHealth, 1 )
						if( IsValid( v.Turret ) ) then 
						
							v.Turret:StopParticles()
						
						end 
						
						if( IsValid( v.Barrel ) ) then 
						
							v.Barrel:StopParticles()
						end 
...
function Meta:NeuroNaval_InflictCrewDamage( )
			
	
	self.MedicHealth = math.Approach( self.MedicHealth, 0, math.random(1,3)*15 ) 
	self.FiremenHealth = math.Approach( self.FiremenHealth, 0,  math.random(1,3)*15 ) 
	self.EngineerHealth = math.Approach( self.EngineerHealth, 0,  math.random(1,3)*15 ) 
...
function Meta:NeuroNaval_UpdateCrewHealth()

	-- print( self.MedicHealth, self.FiremenHealth, self.EngineerHealth )
	self:SetNWFloat("NeuroShips_HealthMedic", self.MedicHealth )
	self:SetNWFloat("NeuroShips_HealthFiremen", self.FiremenHealth )
	self:SetNWFloat("NeuroShips_HealthEngineer", self.EngineerHealth )
end 
...
function Meta:NeuroNaval_DefaultDamage( dmginfo )
	...
	if( IsValid( self.Pilot ) && IsValid( inflictor ) && inflictor:GetClass() == "sent_mini_torpedo" ) then 
		...
		local side = "port"
		local _amt
		local _max = self.InitialHealth 
		local _10th  = .1 * _max 
		
		if( dmg <= 2* _10th ) then
			_amt = "Minor"
		elseif( dmg > 2 * _10th ) then 
			_amt = "Medium" 
		elseif( dmg > 4 * _10th ) then 
			_amt = "Major" 
		elseif( dmg > 5 * _10th ) then
			_amt = "Critical"
		end 
			...
			if( IsValid( v.Base ) && !v.Destroyed ) then 
				...
				if( dist < 8500 ) then 
					...
					if( v.HealthVal <= 0 ) then 
						...
						--ParticleEffectAttach( "fire_b", PATTACH_ABSORIGIN_FOLLOW, v.Base, 0 ) --annoying default fire
						timer.Simple( math.random(1,1), function() --instant explosion effect, no lasting fire
	...
	if( dmg > 350 ) then
		...
		self:NeuroNaval_InflictCrewDamage( )
		...
		if( trace.Hit && IsValid( trace.Entity ) && trace.Entity == self  && self.HealthVal < .85 * self.InitialHealth ) then
			...
			if( self.DamagePoints == {} ) then
				...
				waterballast:GetPhysicsObject():SetMass( self:GetPhysicsObject():GetMass() * mass)
				...
				table.insert( self.Ballasts, waterballast )
...
function Meta:NeuroNaval_DefaultCollisionCallback( data, physobj )
	...
	if( data.HitEntity && data.HitEntity != self.Owner && data.HitEntity.Owner != self.Owner && ( data.HitEntity.IsMicroCruiser || data.HitEntity == game.GetWorld() ) ) then 
		
		-- print( self, "Speed: "..math.floor(data.Speed), data.DeltaTime, data.HitForce )
		if( data.DeltaTime > 0.1 && data.Speed > 0 ) then 
			local impactdamage = ( self.OverrideImpactDamage or  math.random(10,35) )
			...
			self.HealthVal = self.HealthVal - impactdamage*data.Speed 
			self:SetNWInt("health",math.floor(self.HealthVal))
...
function Meta:NeuroNaval_DefaultCruiserInit()
	...
	self.EngineerHealth = 500 or self.EngineerHealth 
	self.FiremenHealth = 500 or self.FiremenHealth 
	self.MedicHealth = 500 or self.MedicHealth 
	...
	if ( self.RadarPositions ) then
	 ...
	 if( self.WeaponSystems ) then
		...
		for k,v in pairs( self.WeaponSystems ) do 
			...
			self.Weaponry[k].InitialHealth = self.InitialHealth*.25 
			self.Weaponry[k].HealthVal = self.InitialHealth*.25
	...
	if( self.ExteriorParts ) then 
		
		self.ShipParts = {}
		for i=1,#self.ExteriorParts do 
			
			self.ShipParts[i] = ents.Create("boat_part")
			self.ShipParts[i]:SetModel( self.ExteriorParts[i] )
			self.ShipParts[i]:SetAngles( self:GetAngles() )
			self.ShipParts[i]:SetPos( self:GetPos() )
			self.ShipParts[i].InitialHealth = self.InitialHealth
			self.ShipParts[i].HealthVal = self.HealthVal
			self.ShipParts[i]:Spawn()
			self.ShipParts[i]:SetModel( self.ExteriorParts[i] )
			self.ShipParts[i]:SetMoveType( MOVETYPE_VPHYSICS )
			self.ShipParts[i]:PhysicsInit( MOVETYPE_VPHYSICS )
			self.ShipParts[i]:SetSolid( SOLID_VPHYSICS )	
			self.ShipParts[i]:Activate()
			self.ShipParts[i]:GetPhysicsObject():SetMass( self.PhysObj:GetMass() )
			self.ShipParts[i]:GetPhysicsObject():SetBuoyancyRatio( self.BuoyancyRatio )
			self.ShipParts[i]:GetPhysicsObject():SetDamping( self.DampFactor, self.DampFactor )
			self.ShipParts[i].Owner = self 
			self.ShipParts[i].IsShipHullSegment = true 
			local weldEnt = self 
			if( i == #self.ExteriorParts ) then 
				weldEnt = self.ShipParts[#self.ExteriorParts-1]
			end 
			
			self.ShipParts[i].Weld = constraint.Weld( weldEnt, self.ShipParts[i], 0, 0, 0, true, false )
			
			
			self:DeleteOnRemove( self.ShipParts[i] )
		end 
		
		self.ShipParts[1].DamageScaleFactor = .1 
		-- self.RipOrder = {}
