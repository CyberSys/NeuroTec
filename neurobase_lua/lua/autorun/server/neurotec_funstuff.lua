...
hook.Add("PlayerLoadout", "NeuroWeapons_massacreMode", function( ply )

	if( IsValid( CHAINSAW_CORP ) ) then 
		
		-- local k = math.random(1,#player.GetAll() )
		-- local pos = CHAINSAW_CORP:LocalToWorld( Vector( math.sin( 360/k ) * 256, math.cos( 360/k ) * 266, 4505 ) ) 
		ply:StripWeapons()
		ply:SetPos( CHAINSAW_CORP:LocalToWorld( Vector( 272+math.random(-100,0), math.random( -26, 350 ), 4600 ) ) )
		ply:Give("weapon_neurowep_chainsaw")
...
hook.Add("PlayerDeath", "NeuroWeapons_ChainsawMinigame", function( ply )
...
hook.Add( "PlayerSay", "NeuroPlanes_ChatCommands", function( ply, txt, team )
	...
	local said = string.Explode( " ", txt:lower() )
	...
	local cmds = { 
					...
					{ 
						...
						Callback = function( ply, txt, team )	
										...
										if( IsValid( ride ) ) then 
											...
											if( !IsValid( ride.Neuro_aerobaticSprite ) ) then 
												
												-- local prop = ride 
												local color = Color(math.random(150,255),math.random(150,255),math.random(150,255),200)
												if( ride.Ailerons ) then 
													
													for i=1,#ride.Ailerons do 
														local ail = ride.Ailerons[i]
														if( IsValid( ail ) ) then 
														
															ail.Neuro_aerobaticSprite = util.SpriteTrail( ail, 0,color, false, 1, 100, 15, math.sin(CurTime()) / math.pi * 1, "trails/smoke.vmt")
															ride.Neuro_aerobaticSprite = ail.Neuro_aerobaticSprite 
													...
													local col = math.Rand( 0, 1) / 1 * 255 
													ride.Neuro_aerobaticSprite = util.SpriteTrail( ride, 0,color, false, 1, 100, 15, math.sin(CurTime()) / math.pi * 1, "trails/smoke.vmt")
												...
												if( ride.Ailerons ) then 
													...
													for i=1,#ride.Ailerons do 
														local ail = ride.Ailerons[i]
														if( IsValid( ail ) && IsValid( ail.Neuro_aerobaticSprite ) ) then 
														
															ail.Neuro_aerobaticSprite:Remove()
															ride.Neuro_aerobaticSprite = NULL 
													...
													ride.Neuro_aerobaticSprite:Remove()
					...
					{ 
						...
						Callback = function( ply, txt, team )
										...
										if( ply:IsAdmin() || ply:IsBot() ) then
											...
											local rnd = Vector( math.random(-150,150), math.random(-150,150), 0)
											local Target = FindPlayerByPartialName( ply, string.Explode( " ", txt )[2] )
											
											local anvil = ents.Create("prop_physics")
											anvil:SetPos( Target:GetPos() + Target:GetRight()* rnd + Target:GetUp() * 2000 )
											anvil:SetModel("models/sillirion/anvil.mdl")
											--anvil:SetOwner( ply )
											anvil:SetPhysicsAttacker( ply )
											anvil:Spawn()
											anvil:Fire( "kill", "", 7 )
											anvil:EmitSound( "funny.wav", 511, 140 )
										end
					...
					{ 
						Var = "!chainsawmassacre", 
						Callback = function( ply, txt, team )	
										
										if( !ply:IsSuperAdmin() ) then return end
										if( IsValid( CHAINSAW_CORP ) ) then ply:PrintMessage( HUD_PRINTCENTER, "NO YOU MORON! FINISH WHAT YOU STARTED" ) return end 
										local tr,trace={},{}
										tr.start = ply:GetPos()
										...
										local oldFalldamage = GetConVarNumber("mp_Falldamage")
										local corp = ents.Create("prop_physics")
										corp:SetPos( ply:GetPos() )
										corp:SetAngles( Angle(0,0,0))
										corp:SetModel("models/props_phx/huge/evildisc_corp.mdl")
										corp:SetMoveType( MOVETYPE_NONE )
										corp:SetSolid( SOLID_VPHYSICS )
										corp:Spawn()
										corp:GetPhysicsObject():EnableMotion( false )
										corp:Fire("kill","",61)
										corp.PhysgunDisabled  = true 
										
										CHAINSAW_CORP = corp 
										RunConsoleCommand("mp_Falldamage",2 )
										
										-- NeuroTecChainsawParty = {}
										local kickBotsAfterMassacre = false
										
										timer.Simple( 59, function()
											if( !IsValid( corp ) ) then return end 
													corp:PlayWorldSound( "ambient/athmosphere/thunder2.wav")
													ParticleEffect("fireball_explosion", corp:GetPos() + corp:GetUp() * 1,corp:GetAngles(), nil )
											end ) 	
										
										timer.Simple( 45, function()
											if( !IsValid( corp ) ) then return end 
													-- ParticleEffect("fireball_explosion", corp:GetPos() + corp:GetUp() * 1, Angle(0,0,0), nil )
													corp:SetMoveType( MOVETYPE_VPHYSICS )
													corp:GetPhysicsObject():EnableMotion( true )
													corp:GetPhysicsObject():Wake()
													corp:GetPhysicsObject():AddAngleVelocity( Vector( 145, 0,0 ) )
													corp:SetAngles( Angle( 5,0,0 ) )
													PrintMessage( HUD_PRINTCENTER, "THE END IS NIGH - 15 SECONDS REMAINING" )
													-- player.GetAll()[math.random(1,num)]:EmitSound("plats/skylift_move.wav", 511, 100 )
													
											end ) 
														
										timer.Simple( 60, function()
											if( !IsValid( corp ) ) then return end 
													corp:PlayWorldSound( "ambient/explosions/explode_7.wav")
													ParticleEffect("fireball_explosion", corp:GetPos() + corp:GetUp() * 3000, corp:GetAngles(), nil )
													timer.Simple( 5, function() 
														RunConsoleCommand( "mp_Falldamage", oldFalldamage )
													end )
													
													local highScore = 0 
													local winner = NULL 
													for k,v in pairs( player.GetAll() ) do 
														if( v:Frags() > highScore ) then 
															highScore = v:Frags()
															winner = v
															v:SetFrags( v.OldFrags or 0  )
															v:SetDeaths( v.OldDeaths or 0 )
														end 
													end 
													PrintMessage( HUD_PRINTTALK, winner:Name().." won the massacre with "..highScore.." kills!" )
													PrintMessage( HUD_PRINTCENTER, winner:Name().." won the massacre with "..highScore.." kills!"  )
													
													if( kickBotsAfterMassacre ) then 
														for k,v in pairs( player.GetBots() ) do v:Kick("Massacre over, your job is done") end 
													end 
											end ) 
											
										ply:SetPos( corp:LocalToWorld( Vector( 321, -319, 4690 ) ) )
										local numbots = #player.GetBots()
										if( numbots == 0 ) then 
											for i=1,5 do 
												RunConsoleCommand("bot")
											end 
											kickBotsAfterMassacre = true 
											
										end
											
										for k,v in pairs( player.GetAll() ) do 
											local r = v:GetScriptedVehicle()
											local j = v:GetVehicle()
											if( IsValid( r ) ) then r:Remove() end 
											if( IsValid( j ) ) then j:Remove() end 
											v:Spawn()
											-- if( k == num ) then 
												-- v:EmitSound("music/stingers/industrial_suspense2.wav", 511, 100 )
											-- end 
											v.OldFrags = v:Frags()
											v.OldDeaths = v:Deaths()
											v:SetFrags( 0 )
											v:SetDeaths( 0 )
											v:PrintMessage( HUD_PRINTCENTER, "PREPARE TO DIE - YOU HAVE 60 SECONDS" )
											v:StripWeapons()
											v:SetMoveType( MOVETYPE_WALK )
											v:Give("weapon_neurowep_chainsaw")
											v:SetPos( corp:LocalToWorld( Vector( math.sin( 360/k ) * 256, math.cos( 360/k ) * 266, 4505 ) ) )
											if( v:IsBot() ) then	
												v.Target = player.GetAll()[math.random(1,num)]
											end 
					...
					{ 
						...
						Callback = function( ply, txt, team )	
										...
										ply:PlayWorldSound( "misc/Tactical_Nuke_Incoming.wav" )
										ply:EmitSound("misc/Tactical_Nuke_Incoming.wav" , 511, 100 )
					...
					Callback = function( ply, txt, team ) 
						...
						if( IsValid( Target ) ) then
							...
							if( IsValid( Ride ) ) then
								...
								Ride.HealthVal = 0 
								util.BlastDamage( Ride, Ride, Ride:GetPos(), 16, 16 )
					...
					Callback = function( ply, txt, team ) 
						...
						if( IsValid( Target ) ) then
							...
							local Ride = Target:GetScriptedVehicle().Wings[1]
							local Ride2 = Target:GetScriptedVehicle().Wings[2]
					...
					Callback = function( ply, txt, team ) 
						...
						if( IsValid( Target ) ) then
							...
							local Ride = Target:GetScriptedVehicle()
							Ride.Propeller = NULL 
							Ride.PropellerAxis = NULL
							
							-- print("walla", Ride.EngineBones )
							-- if( IsValid( Ride ) && IsValid( Ride.Propeller ) ) then
							-- if( Ride.EngineBones ) then 
								-- print("walla2")
								-- for k,v in ipairs( Ride.EngineBones ) do 
									
									-- local bp,ba = Ride:GetBonePosition( v.Index ) 
									
									-- v.HealthVal = 0 
									-- util.BlastDamage( game.GetWorld(), game.GetWorld(), bp, 4, 4 )
					...
					Callback = function( ply, txt, team ) 
						...
						for i=1,20 do 
							...
							local f = ents.Create("sent_interceptor")
							f:SetPos( ply:GetPos() + Vector( math.sin( i * 18 ) * 1500, math.cos( i * 18 ) * 1500, 500 ) )
							f:SetAngles( Angle( 0, i * 18, 0 ) ) 
							f:Spawn()
							f:Use( ply, ply, 0, 0 )
							if( IsValid( Target ) ) then 	
					...
					};
					{
						...
						Callback = function( ply, txt, team ) 
							...
							if( ply:IsAdmin() || ply:IsBot() ) then
								...
								-- end
								ParticleEffect( "microplane_bomber_explosion", _end:GetPos(), Angle( 0,0,0 ), nil )
												...
												timer.Simple( 3, function()
													...
													if( IsValid( Target ) ) then
														...
														for i=1,15 do -- cheap
															...
															NeuroDirectSound( Target, "ambient/creatures/town_child_scream1.wav" )
										...
										if( ply:IsAdmin() || ply:IsBot() ) then
												...
												for k,v in pairs( player.GetAll() ) do
													...
													if( ( v:GetPos() - Target:GetPos() ):Length() < 25000 ) then
														...
														NeuroDirectSound( v, "ambient/atmosphere/thunder2.wav" )
														NeuroDirectSound( v, "ambient/explosions/explode_7.wav" )
	...
	for i=1,#cmds do	
		...
		if( table.HasValue( said,  cmds[i].Var:lower() ) ) then
