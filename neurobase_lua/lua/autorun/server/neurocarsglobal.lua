...
concommand.Add( "neurotec_swapseat", function( ply, cmd, args )
	...
	if( IsValid( seatparent ) && seatparent.ValidSeats ) then
		...
		if ( seatnumber != nil && type(seatnumber) == "number" && seatnumber <= #seatparent.ValidSeats  ) then
			...
			if( IsValid( NewSeat ) && NewSeat:GetClass() == "prop_vehicle_prisoner_pod" && !IsValid( NewSeat:GetDriver() ) ) then
					-- print("HARAM")
...
concommand.Add("ntespv",function( ply, cmd, args ) 
	...
	for k, v in pairs( scripted_ents.GetSpawnable( ) )  do
		...
		if( string.lower(v.ClassName) ==  string.lower(args[1]) ) then
			...
			if( v.Category == "NeuroTec Micro" ) then 	
			
				pos = ply:GetPos() + Vector(0,0,1500) 
			
			end 
			
			if( v.IsMicroHelicopter ) then 
...
function Meta:Jet_LockOnMethod()
	...
	if ( logic && logic2 && logic3 && !IsValid( self.NeuroTarget ) && e:GetOwner() != self && e:GetOwner() != self.Pilot && e:GetClass() != self:GetClass() ) then
		
		if( IsValid( e.TailRotor ) ) then
			
			self:SetNeuroTarget( e.TailRotor )
				
			return
			
		end
		
		self:SetNeuroTarget( e )
...
function Meta:Turret_LockOnMethod()
	...
	if ( logic && logic2 && !IsValid( self.NeuroTarget ) && e:GetOwner() != self && e:GetOwner() != self.Pilot && e:GetClass() != self:GetClass() ) then
...
function Meta:Micro_FireCannons()
	...
	if( IsValid( g ) && !g.Destroyed ) then
		...
		if( self.PCFMuzzle ) then
				...
				if( self.GunHasBones ) then 
				
					ParticleEffectAttach( self.Muzzle, PATTACH_POINT_FOLLOW, g, 1 )
				
				else
				
					ParticleEffect( self.Muzzle or "microplane_mG_muzzleflash", g:GetPos()+g:GetForward()*16,  bullet:GetAngles(), bullet )		
		...
		bullet:GetPhysicsObject():SetVelocity( bullet:GetForward() * 15000 )
		end)
	end
...
function Meta:Jet_FireMultiBarrel()
	...
	local i = self.MinigunIndex
		...
		bullet.Callback    = function ( a, b, c )
								...
								-- print( b.Entity )
								if( b.Entity && b.Entity.HealthVal ) then 
									
									local smoke = ents.Create("neuro_impact_smoke")
									smoke:SetPos( b.HitPos )
									smoke:SetAngles( b.HitNormal:Angle() )
									smoke:SetParent( b.Entity ) 
									smoke:Spawn()
									smoke:Fire("kill","",5)
		...
		if( self.Muzzle && !self.PCFMuzzle ) then
			...
			if( self.GunHasBones ) then 
				...
				-- print("lahme")
				ParticleEffectAttach( "microplane_mG_muzzleflash", PATTACH_POINT_FOLLOW, self.Miniguns[i], 1 )
				-- timer.Simple(0.1,function() if(IsValid( self.Miniguns[i] ) ) then 
				
						-- self.Miniguns[i]:StopParticles()
...
function Meta:SonicBoomTicker()
	...
	if ( math.floor( self:GetVelocity():Length() ) > ( 1.9 * 1224 ) ) then
	...
	if ( ( math.floor( self:GetVelocity():Length() ) >= ( 1.8 * 1224 ) ) && 
		 ( math.floor( self:GetVelocity():Length() ) <= ( 1.8 * 1224+96 ) )  ) then -- Don't want the vapor cloud on deceleration //StarChick971 actually we do as we reach mcrit water vapor will be visible again //hoffa
...
function Meta:NeuroPlanes_CycleThroughJetKeyBinds()
	...
	-- Clear Target 
	if ( self.Pilot:KeyDown( IN_WALK ) && IsValid( self.NeuroTarget ) ) then
	...
	if ( self.Pilot:KeyDown( IN_ATTACK ) ) then
		...
		if ( self.LastPrimaryAttack + self.PrimaryCooldown <= CurTime() && self.OverHeated + 5 <= CurTime() ) then
			...
			if( iLoveLua == "STOP_SOUNDS" ) then return end 
			
			if( self.PrimarySound && self.ContigiousFiringLoop && !self.DontPlayShootSounds ) then
			...
			if( self.FiringTimer >= ( self.PrimaryMaxShots or 25 ) ) then
				...
				if( !self.OverheatCounter ) then self.OverheatCounter = 0 end 
				self.OverheatCounter = self.OverheatCounter + 1 
				
				if( self.Miniguns && self.Miniguns[self.MinigunIndex]  && IsValid( self.Miniguns[self.MinigunIndex] ) ) then 
				
					local g = self.Miniguns[self.MinigunIndex]
					
					if( self.OverheatCounter && self.OverheatCounter >= 5 ) then 
...
function Meta:NeuroPlanes_CycleThroughHeliKeyBinds()
	...
	-- Clear Target 
	if ( self.Pilot:KeyDown( IN_SPEED ) && IsValid( self.NeuroTarget ) ) then
...
function Meta:NeuroPlanes_FireRobot( wep, id )
	...
	if ( wep.Type == "Homing" && !self.NeuroTarget ) then
	...
	r.TargetNeuro = self.NeuroTarget
	r.Pointer = pilot
	...
	if( wep.Type == "Shell" ) then
		...
		if( self.PhysShellSpawnOffset ) then 
			
			r:SetPos( r:GetPos() + r:GetForward() * self.PhysShellSpawnOffset )
			-- print("offset")
		end 
		local muzzlepos = wep 
	...
	if( wep.Type == "Shell" ) then
		...
		r:SetCollisionGroup( COLLISION_GROUP_INTERACTIVE )
		r:GetPhysicsObject():SetVelocity( r:GetForward() * 10000 )
		r:GetPhysicsObject():EnableDrag( true )
...
function Meta:NeuroPlanes_missileStorm( wep )
	...
	s.TargetNeuro = self.NeuroTarget
...
function Meta:NeuroPlanes_BlowWelds( pos, radius )
	...
	for k,v in pairs( ents.FindInSphere( self:GetPos(), radius or 16 ) ) do 
...
function Meta:RocketBarrage( wep )
	...
	r:SetPos( wep:GetPos() + wep:GetForward() * 8)
	r:SetModel(  "models/hoffa/neuroplanes/american/ah1/ah1_rockets.mdl" )
	r:SetAngles( wep:GetAngles() + Angle( math.Rand(-.4,.4),math.Rand(-.4,.4),math.Rand(-.4,.4) ) )
	r:SetPhysicsAttacker( self.Pilot, 999999 )
	r:SetCollisionGroup( 0 )
	r:Spawn()
	r:Fire("Kill","",15)
	r:SetOwner( self )
	r.Owner = self.Pilot
	timer.Simple( 0.5, function() if( IsValid( r ) ) then 
...
function Meta:ClearTarget()
	...
	self.NeuroTarget = NULL -- Clear Target
	self:SetNetworkedEntity( "Target", NULL )
	
end

function Meta:SetNeuroTarget( e )
	...
	if( self.LastRadarScan != nil ) then
		...
		if ( self.LastRadarScan + 1 <= CurTime() ) then
			...
			for k,v in pairs( ents.GetAll() ) do
				...
				if ( ( IsValid( v ) && IsValid( v.TargetNeuro ) && v.TargetNeuro == self ) || ( v.TargetNeuro == self.Pilot && string.find(string.lower(v:GetClass()),"neuro") == nil ) ) then
				
					v:SetNetworkedEntity("Target", v.TargetNeuro )
...
function Meta:SpawnFlare()
	...
	if( !self.RocketVisuals ) then return end 
	if( self.IsHelicopter || self.Wings ) then return end 
...
function Meta:Neuro_DealDamage( Type, Damage, Pos, Radius, DoEffect, Effect )
	...
	for k, v in pairs( ents.GetAll( ) ) do  
		...
		if ( v && IsValid( v ) && ( v:IsNPC() || v:IsPlayer() ) && v:Health( ) > 0  ) then  
			
			local p = v:GetPos( )+v:GetUp()*8 
			
			if ( p:Distance( Pos ) <= Radius ) then  

				local t,tr = {},{}
				t.start = Pos+v:GetUp()*8
				t.endpos = p
				t.mask = MASK_SOLID
				t.filter = self
				tr = util.TraceLine( t )
...
function Meta:ScanForEnemies()
	...
	for k,v in pairs( ents.GetAll() ) do
		...
		if (v.Owner != self.Owner&&v.Owner != self && v != self && v:GetClass() != self:GetClass() && v:GetClass() != "npc_missile_homer" ) then
		...
		if ( self && !IsValid( self.NeuroTarget ) ) then --better safe than sorry
			
			if( IsValid( self.CycleTarget ) ) then
			
				self.NeuroTarget = self.CycleTarget
...
function Meta:CheckLOS(ent)
	...
	if ( self.NeuroTarget == ent && ent:WaterLevel() > 2 ) then
