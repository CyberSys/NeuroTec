...
function DefaultPropPlaneCView( ply, Origin, Angles, Fov )
	...
	if( IsValid( plane ) && ply:GetNetworkedBool( "InFlight", false )  ) then
		...
		if( GetConVarNumber("jet_cockpitview") > 0 ) then
			...
			if( plane.PassengerPos && ply:GetNWBool("MicroPassenger") ) then 
				
				pos = plane:LocalToWorld( plane.PassengerPos )
	...
	*/
		...
		if( ply:GetNWBool("MouseAim") && GetConVarNumber("jet_oldaim") == 0 && !plane.IsHelicopter ) then 
			
			if( GetConVarNumber("jet_cockpitview") == 0 ) then 
			
				pos = LerpVector( 0.5, pos, plane:GetPos() + plane:GetForward() * -plane.CamDist + plane:GetUp() * plane.CamUp  )
			
			end 
			
			ang = LerpAngle( .71,  ang, plane:GetAngles() )
			ang.r = plane:GetAngles().r
			
		end 
		
		if( plane.MinClimb && plane.MaxClimb || plane.IsHelicopter ) then
		...
		if( plane:GetNWBool("SpinningOut") && GetConVarNumber("jet_cockpitview") > 0 ) then 	
			ang = LerpAngle( 0.99, ang, plane:GetAngles() + AngleRand() * .005 )
		end 

		-- if( math.abs( ang.r ) > 89 ) then 
			
			 -- ang = plane:GetAngles() - ang
		-- else
