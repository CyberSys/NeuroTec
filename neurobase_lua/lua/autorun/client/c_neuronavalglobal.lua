...
local SHIP_VELOCITY_IN_KNOTS = 1.73125 
...
function Meta:DefaultMicroShitExhaust( )
	...
	if( self.PropellerPos && self:WaterLevel()>0 && self:GetNWFloat("Throttle") != 0 ) then 
		...
		for i=1,count do 
			...
			local particle = self.Emitter:Add( "effects/splash1", self:LocalToWorld( p ) + self:GetForward()*-10 )
			if ( particle ) then
			-- print("?=?=")
	...
	if( !self.LastExhaustEmit ) then 
		self.LastExhaustEmit = CurTime() 
		
	end 
	
	if( self.LastExhaustEmit + 0.1 >= CurTime() ) then return end 
	self.LastExhaustEmit = CurTime() 
...
-- local particle = self.Emitter:Add( "")
	...
	if( ( scale > .15 || scale < -.15 ) && self:GetVelocity():Length()> 30 && !self.IsMicroSubmarine ) then 
		...
		local particle = self.Emitter:Add( "effects/splash"..math.random(1,2), self:LocalToWorld( pos )  )
		if ( particle  && self:WaterLevel() < 3 ) then
		-- print("?=?=")
			particle:SetVelocity( Vector(0,0,65 + scale * 55 ) )
			particle:SetDieTime( math.Rand( 1, 3 ) )
			particle:SetStartAlpha( math.Rand( 25, 45 ) )
			particle:SetEndAlpha( 0 )
			particle:SetStartSize( math.Rand( 4, 7 ) )
			particle:SetEndSize( math.Rand( 24, 51 ) ) 
			particle:SetRoll( math.Rand(-11.1, 11.1) )
			particle:SetRollDelta( math.Rand(-1, 1) )
			particle:SetColor( 255,255,255 ) 
			particle:SetAirResistance( 150 ) 
			particle:SetCollide( true )
			particle:SetGravity(Vector(0,0,-100) +  ( self:GetForward()*-100 * scale + self:GetRight()*math.random(-395,395) )  )
...
local CrewFolders = {}
CrewFolders["Japan"] = "materials/vgui/ui/crew/japan/"
CrewFolders["Germany"] = "materials/vgui/ui/crew/germany/"
CrewFolders["Russia"] = "materials/vgui/ui/crew/russia/"
CrewFolders["USA"] = "materials/vgui/ui/crew/usa/"
CrewFolders["United Kingdom"] = "materials/vgui/ui/crew/united_kingdom/"
...
function Meta:DefaultNavalClientInit()
	...
	if( self.Country && CrewFolders[self.Country] != nil ) then 
		
		self.Crew = {}
		local pics, path = file.Find( CrewFolders[self.Country].."*.png", "GAME" )
		for i=1,3 do 
		 
			if( pics[i] ) then 
				
				local country = CrewFolders[self.Country]..pics[i]
		
				self.Crew[i] = {}
				self.Crew[i].Pic = Material( country )
				self.Crew[i].Health = 500
				self.Crew[i].HealthMax = 500
	...
	if( self.ExteriorParts ) then 	
		...
		-- self.PartStart.x = self.PartStart.x + length / 2.5
		self.PartLength = self.PartLength*(#self.ExteriorParts-1)
...
net.Receive("NeuroNavalCooldown", function( len, pl ) 
	...
	if( IsValid( boat ) && boat.Weaponry ) then 
		...
		if( #boat.WeaponSystems >= idx && boat.Weaponry[idx] ) then 
...
Naval.Target = NULL
local W,H= ScrW(),ScrH()
local function getPosInTime( speed, pos, t )
	
	return Vector( speed.x * t + pos.x, speed.y * t + pos.y, .5 * 600 * t * t + speed.z + pos.z )  

end 

function NeuroTrajectorySegment( startpos, direction, speed, timeSegment )
	
	-- local last = NeuroTrajectorySegment( startpos, direction, speed, timeSegment - 1 )
	local newpos = startpos + direction * speed * timeSegment * 600 * timeSegment
	return newpos
	
end 

hook.Add("PostDrawOpaqueRenderables", "neuronaval_renderweapon_ind", function()
	local ply = LocalPlayer()
	local boat = ply:GetScriptedVehicle()
			...
			for k,v in ipairs( weapons ) do 
				...
				if( v.WeaponGroup == SelectedWeapon ) then 
					...
					local turretstart = v:GetPos()
					 -- local pos1 = turretstart
					 -- local pos2 = turretstart
					 -- local lastpos1 = pos1 
					 -- local lastpos2 = pos2 
					 -- local velo1 = v.Barrel:GetForward() 
					 -- local velo2 = v.Barrel:GetForward() 
							
					-- for i=1,30 do
						 
						 -- pos1 = NeuroTrajectorySegment( pos1, velo1, 400, i )
						 -- pos2 = NeuroTrajectorySegment( pos2, velo2, 400, i-1 )
						 
						  
						-- v.LinePos = getPosInTime( v:GetPos() + v:GetForward() * 44, v.LinePos or turretstart, CurTime()  ) 
						-- v.LinePos2 = getPosInTime( v:GetPos() + v:GetForward() * 44, v.LinePos2 or turretstart, CurTime()-5  ) 
						
						-- local p1,p2 = v.LinePos:ToScreen(), v.LinePos2:ToScreen()
						-- render.DrawLine( pos1, pos2, Color( 255-i*2,255,25,255 ),false )
...
function DrawNavalHUD()
	...
	if ( Naval.Captain:GetNetworkedBool( "InFlight", false ) && Naval.Ship != Naval.Captain) then
...
local repairicons = {
Material( "vgui/ui/crew/medic.png" ),
Material("vgui/ui/crew/fire.png"),
Material("vgui/ui/crew/repair.png")
}
function Naval.DrawCrew()
	
	
	if( Naval.Ship.Crew ) then 
		
		
		local size = 58
		local space = 30
		local healthData = {
			Naval.Ship:GetNWFloat("NeuroShips_HealthMedic"),
			Naval.Ship:GetNWFloat("NeuroShips_HealthFiremen"),
			Naval.Ship:GetNWFloat("NeuroShips_HealthEngineer")}
	...
	-- self:SetNWFloat("NeuroShips_HealthEngineer", self.EngineerHealth )
		...
		for i=1,3 do 
			...
			local factor = healthData[i] / 500
			
			-- surface.SetDrawColor( Color( 25,25,25,100 ))
			-- surface.DrawRect( (size+space) + (#Naval.Ship.Crew*(size+space)) - (size+space) * i,ScrH()-size, size+space, size )
			
			surface.SetDrawColor( Color( 255-(255*factor),255*factor,0,150 ))
			surface.DrawRect( (size+space) + (#Naval.Ship.Crew*(size+space)) - (size+space) * i,ScrH()-size, space/2, (size*factor) )
			
			surface.SetDrawColor( Color( 255,255,255,255 ))
			surface.SetMaterial( Naval.Ship.Crew[i].Pic )
			surface.DrawTexturedRect( (size+space) + (#Naval.Ship.Crew*(size+space)) - (size+space) * i,ScrH()-size, size, size )
			
			surface.SetMaterial( repairicons[i] )
			surface.DrawTexturedRect( (size+size+space/1.5) + (#Naval.Ship.Crew*(size+space)) - (size+space) * i,ScrH()-size, 40, 40 )
...
function Naval.DrawWW2SubInstruments()
	...
	-- local displaySpeed = math.Round( Naval.Ship:GetVelocity():Length() * SHIP_VELOCITY_IN_KNOTS )
	
	-- surface.SetFont( "Electrolize2" )
	-- surface.SetTextColor( 25, 25, 25, 255 )
	-- surface.SetTextPos( ScrW() - telY-10, ScrH()-160  ) 
	-- surface.DrawText( displaySpeed )
