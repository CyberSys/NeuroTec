...
CreateClientConVar("gred_cl_neuro_azerty"		, "0" , true,false)
...
hook.Add( "Think", "NeuroTec_SendKeyBinds", function()
	...
	if( ply.LastNetTick + 0.25 <= CurTime() && IsValid( ply:GetScriptedVehicle() ) ) then
		...
		if GetConVar("gred_cl_neuro_azerty"):GetInt() == 0 then 
			keyleft = KEY_Q 
		else 
			keyleft = KEY_A
		end
...
function Meta:CivAir_CInit()
	...
	self.CamUp = self.CameraUp*3 or 100
...
function Meta:CivAir_Draw()
	...
	local alpha =  math.Rand( 5, 7 )
	local endsize = 0 
	local dietime = 1 
	local r,g,b = math.random(125,155),math.random(125,155),math.random(125,155)
	local startVelocity = Vector()
	...
	if( percentage < 20 ) then	
		...
		alpha = 100
		endsize = 165
		dietime = math.random(8,11)
		r,g,b = math.random(25,45),math.random(25,45),math.random(25,45)
	end
	
	if( self:GetNWBool("EngineOff") ) then return end 
		...
		for i=1,#self.ExhaustPos do
			...
			if ( particle ) then
				...
				if( self.JetExhaust ) then
					...
					if( self.VTOL && self:GetVelocity():Length() ) then 
						
						dir = self:GetUp() * 1 + self:GetForward() * .15
						
					end 
					
					startVelocity =  dir * -300 + Vector( math.Rand( -1.5,1.5),math.Rand( -1.5,1.5),math.Rand( -1.5,1.5)  ) / div
					-- print("lahme")
					...
					startVelocity = self:GetForward() * -25 + Vector( math.Rand( -1.5,1.5),math.Rand( -1.5,1.5),math.Rand( 2.5,15.5)  ) / div
				...
				local startSize = math.Rand( 3, 5 )/div
				local endSize2 =  math.Rand( 8, 16 )/div
				local temp = self:GetNWInt("EngineTemp",0 )
					
				if( pilot:KeyDown( IN_JUMP ) || temp < 25 ) then 
					startSize = math.Rand( 7, 9 )/div
					alpha = 5
					endSize2 = 9
				end 
