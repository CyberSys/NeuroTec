...
CreateClientConVar( "jet_disablecalcview", "0", true, false )
 
hook.Add("CalcView", "!NeuroTecGatpotziMode", function( ply,  pos, ang, fov, nearZ, farZ  )
	if( GetConVarNumber("jet_disablecalcview" ) > 0 && IsValid( ply:GetScriptedVehicle() ) ) then 	
		return false
	end 
end ) 
...
CreateClientConVar( "jet_oldaim", "1", true, false )
...
planeicon_materials["planeicon_3engine_fuselage"] = Material("hud/planeicons/planeicon_2engine_fuselage.png")
planeicon_materials["planeicon_3engine_tail"] = Material("hud/planeicons/planeicon_2engine_tail.png")
planeicon_materials["planeicon_3engine_engine1"] = Material("hud/planeicons/planeicon_2engine_engine1.png")
planeicon_materials["planeicon_3engine_engine2"] = Material("hud/planeicons/planeicon_2engine_engine2.png")
planeicon_materials["planeicon_3engine_engine3"] = Material("hud/planeicons/planeicon_2engine_engine2.png")
planeicon_materials["planeicon_3engine_rightwing"] = Material("hud/planeicons/planeicon_2engine_rightwing.png")
planeicon_materials["planeicon_3engine_leftwing"] = Material("hud/planeicons/planeicon_2engine_leftwing.png")
planeicon_materials["planeicon_3engine_rightaileron"] = Material("hud/planeicons/planeicon_2engine_rightaileron.png")
planeicon_materials["planeicon_3engine_leftaileron"] = Material("hud/planeicons/planeicon_2engine_leftaileron.png")
planeicon_materials["planeicon_3engine_elevator"] = Material("hud/planeicons/planeicon_2engine_elevator.png")
planeicon_materials["planeicon_3engine_rudder"] = Material("hud/planeicons/planeicon_2engine_rudder.png")
...
function inLOS( a, b )
	...
	local trace = util.TraceLine( { start = a:GetPos() + a:GetForward() * 150, endpos = b:GetPos()+b:GetUp()*8, filter = a, mask = MASK_WATER + MASK_SOLID } )
	
	-- print( ( ( trace.Hit && trace.Entity == b ) || !trace.Hit ), b )
...
--	JetFighter.Pilot:StopSound( "lockon/launch.mp3" )
	...
	if( GetConVarNumber("jet_cockpitview") > 0 ) then
		...
		if string.find( JetFighter.Plane.Category,"NeuroTec Micro" ) != nil  then
...
local ai_bg = Material( "ai_back.png" )
local ai_dial = Material( "ai_dial.png" )
local ai_center = Material( "ai_center.png")
local ai_cross = Material( "ai_cross.png")
local compass_bg = Material( "compass_back.png" ) 
local compass_dial = Material( "compass_dial.png" ) 
local vspeed_back = Material("altimeter_back.png")
local vspeed_dial = Material("altimeter_dial.png")
local alt_back = Material("alt_back.png")
local alt_dial = Material("alt_dial.png")
local clock_bg = Material( "clock_bg.png")
local clock_needle = Material( "clock_dial.png")
local airspeed_bg = Material( "airspeed_bg.png")
local throttle_bg = Material( "throttle_bg.png")
local throttle_dial = Material( "throttle_dial.png")
local temp_bg = Material( "oiltemp_bg.png")
local fuel_bg = Material( "fuel_back.png")
-- local throttle_dial = CreateMaterial("throttle_dial", "UnlitGeneric", {
	-- ["$ignorez"] = 0,
	-- ["$vertexcolor"] = 1,
	-- ["$vertexalpha"] = 1,
	-- ["$nolod"] = 1, 
	-- ["$basetexture"] = Material( "throttle_dial.png")
-- })

NeurotecFlightmodules = {} 

function NeurotecFlightmodules.GunFLIR( size, x, y, ent )
...
local rendertarget = GetRenderTarget( "NeuroTec_miniGunscreen", 512, 512, false )	
local cammaterial = CreateMaterial("NeuroTec_microGunDisplay", "UnlitGeneric", {
	["$ignorez"] = 1,
	["$vertexcolor"] = 1,
	["$vertexalpha"] = 1,
	["$nolod"] = 1,
...
function NeurotecFlightmodules.GunCamera( pos, ang, gun, scale )

	local w, h = ScrW(), ScrH() 
	
	camdat.origin = gun:GetPos() + gun:GetForward() * 16 + gun:GetRight() * 8
	camdat.angles = gun:GetAngles()
	camdat.w = w
	camdat.h = h
	camdat.x = 0
	camdat.y = 0
	...
	render.CopyRenderTargetToTexture( old_rendertarget )
	...
	cam.Start3D2D( pos, ang, scale ) 
		...
		surface.DrawTexturedRect( 0, 0, rsize,rsize ) 
	cam.End3D2D()
end

function NeurotecFlightmodules.Clock( size, x, y )

	local seconds = os.date("%S")
	local minutes = os.date("%M")
	local hours = os.date("%H")
	local degHours = 360 / 12 * -hours 
	local degMins = 360 / 60 * -minutes 
	local degSecs = 360 / 60 * -seconds 
	surface.SetMaterial( clock_bg ) 
	surface.DrawTexturedRect( x-size/2,y-size/2, size,size )
	surface.SetMaterial( clock_needle ) 
	surface.DrawTexturedRectRotated( x, y, size/2.5, size, degSecs )
	surface.DrawTexturedRectRotated( x, y, size/1.5, size, degMins )
	surface.DrawTexturedRectRotated( x, y, size, size/1.25, degHours+degMins/12 )
	
end 
function NeurotecFlightmodules.CockpitLight( lightData, ent )
	
	local pos = lightData.Pos 
	local dlight = DynamicLight( ent:EntIndex() )
	local c = lightData.Col 
	if ( dlight ) then
		dlight.pos = ent:LocalToWorld( pos )
		dlight.r = c.r
		dlight.g = c.g
		dlight.b = c.b
		dlight.brightness = lightData.Bright
		dlight.Decay = .1
		dlight.Size = lightData.Size
	...
	cam.Start3D( ent:LocalToWorld( pos ), ang ) -- Start the 3D function so we can draw onto the screen.
		render.SetMaterial( lightData.Mat ) -- Tell render what material we want, in this case the flash from the gravgun
		render.DrawSprite( ent:LocalToWorld( pos ), 4, 4, c ) -- Draw the sprite in the middle of the map, at 16x16 in it's original colour with full alpha.
	cam.End3D()
end 

function NeurotecFlightmodules.EngineTemp( size, x, y, ent )

	local temp = ent:GetNWInt("EngineTemp",0 )
	local throttle = ent:GetNWInt("HelicopterThrottle", 0)
	if( throttle >= 100 ) then 
		temp = temp / 2 
	end 
	
	surface.SetMaterial( temp_bg ) 
	surface.DrawTexturedRect( x-size/2,y-size/2, size,size )
	surface.SetMaterial( clock_needle ) 
	surface.DrawTexturedRectRotated( x, y, size, size, -180 -temp )
	
end 

function NeurotecFlightmodules.FuelCounter( size, x, y, ent )

	local temp = ent:GetNWInt("NeuroFuelCounter", 1800 )
	local ang = 160 / 1800 * temp 
	local rate = 60 
	local throt = ent:GetNWInt("HelicopterThrottle")
	if( throt >= 100 ) then 	
		rate = 120 
	end 
	
	if( ent:GetNWBool("LosingFuel") ) then 
		rate = 240 
	end
	
	surface.SetMaterial( fuel_bg ) 
	surface.DrawTexturedRect( x-size/2,y-size/2, size,size )
	surface.SetMaterial( clock_needle ) 
	surface.DrawTexturedRectRotated( x, y, size, size, 160 -ang )
	
	surface.SetFont( "Trebuchet24" )
	surface.SetTextColor( 255, 255, 255, 255 )
	surface.SetTextPos( x-40, y+15 )
	surface.DrawText( math.floor(temp/rate).." mins" )
	
end 

function NeurotecFlightmodules.Throttle( size, x, y, ent ) 

	local throttle = size / 130 * ent:GetNWInt("HelicopterThrottle")
	-- print( throttle) 
	local throttleX = math.Clamp( throttle, 0, size-(size/5) )
	surface.SetMaterial( throttle_bg ) 
	surface.DrawTexturedRect( x-size/2,y-size/2, size/2,size )

	surface.SetMaterial( throttle_dial ) 
	surface.DrawTexturedRectRotated( x-size/4, y-throttleX, size/2, size, 0 )
	
end 


function NeurotecFlightmodules.Altimeter( size, x, y, ent ) 
	...
	local dist = math.floor( ent:GetPos().z - dtrace.HitPos.z  )
	local _max = dist / ( ent.ServiceCeiling or 36000 )
	local speed = math.floor( ent:GetVelocity().z )
	
	surface.SetMaterial( alt_back ) 
	surface.DrawTexturedRect( x-size/2,y-size/2, size,size )

	surface.SetMaterial( alt_dial ) 
	surface.DrawTexturedRectRotated( x, y, size, size, -360 * _max )
	
	surface.SetFont( "DermaLarge" )
	surface.SetTextColor( 255, 255, 255, 255 )
	surface.SetTextPos( x-25, y+62 )
	surface.DrawText( dist )
	
	surface.SetFont( "DermaLarge" )
	surface.SetTextColor( 255, 255, 255, 255 )
	surface.SetTextPos( x-65, y-28 )
	surface.DrawText( math.Round( _max * 100 )  )	
	
	surface.SetFont( "DermaLarge" )
	surface.SetTextColor( 255, 255, 255, 255 )
	local displayValue = math.Clamp( math.Round( ( ent.ServiceCeiling or 36000 ) - dist ), 0, 9999999 )
	if( displayValue < 1000 ) then 
		surface.SetTextColor( 255, 25, 25, 255 )
	end 
	surface.SetTextPos( x+26, y-28 )
	surface.DrawText( displayValue  )


end 

function NeurotecFlightmodules.VSpeedIndicator( size, x, y, ent )

	local speed = math.floor( ent:GetVelocity().z )
	
	surface.SetMaterial( vspeed_back ) 
	surface.DrawTexturedRect( x-size/2,y-size/2, size,size )

	surface.SetMaterial( vspeed_dial ) 
	surface.DrawTexturedRectRotated( x, y, size, size, -math.Clamp( speed / 10, -45, 45 ) )
	
	surface.SetFont( "DermaLarge" )
	surface.SetTextColor( 255, 255, 255, 255 )
	surface.SetTextPos( x-25, y+62 )
	surface.DrawText( speed/10 )
	
end 

function NeurotecFlightmodules.Airspeed( size, x, y, min, max  ) 

	surface.SetMaterial( airspeed_bg ) 
	surface.DrawTexturedRect( x-size/2,y-size/2, size,size )
	
	local ang = 300 * math.Clamp(  min / max, 0, 1 )
	
	surface.SetMaterial( vspeed_dial ) 
	surface.DrawTexturedRectRotated( x, y, size, size, 60-ang )
	
end 

function NeurotecFlightmodules.Compass( size, x, y, a ) 

	surface.SetMaterial( compass_bg ) 
	surface.DrawTexturedRect( x-size/2,y-size/2, size,size )

	surface.SetMaterial( compass_dial ) 
	surface.DrawTexturedRectRotated( x, y, size, size, -a.y )
	
end 

function NeurotecFlightmodules.AttitudeIndicator( size, x, y, a ) 

		surface.SetDrawColor( 255, 255, 255, 255 )
		
		surface.SetMaterial( ai_bg ) 
		surface.DrawTexturedRect( x-size/2,y-size/2, size,size )
		
		local theta = math.rad( -a.r )
		local theta2 = math.rad( -a.p )
		local nx = x * math.cos(theta ) - y * math.sin(theta)
		local ny = y * math.sin(theta2 ) - x * math.cos( theta2 )
		surface.SetMaterial( ai_center ) 
		local offs = a.p 
		if( math.abs( a.p ) > 89 || math.abs( a.r ) > 89 ) then 
			offs = -a.p 
		end 
		-- surface.DrawTexturedRectRotated( math.Clamp( x - nx/4+size/4, x-45, x+45) , math.Clamp( x - ny/4-size/4, y-45, y+45), size, size, a.r )
		surface.DrawTexturedRectRotated( x, math.Clamp( y+offs, y-55, y+55), size, size, a.r )
		-- surface.DrawTexturedRectRotated( x+math.sin(math.rad(a.p))*-a.p, y+math.cos(math.rad(a.r))*-a.r, size, size, a.r )
		
		surface.SetMaterial(  ai_dial )
		surface.DrawTexturedRectRotated( x, y, size, size, a.r )
		
		surface.SetMaterial(  ai_cross )
		surface.DrawTexturedRectRotated( x, y, size, size, 0 )
	

end 

function JetFighter.AttitudeIndicator()
	
	local ply = LocalPlayer()
	local helo = ply:GetScriptedVehicle()
	local size = 256 
	local x,y = ScrW()-size/2, ScrH()-size/2 
		
	if( IsValid( helo ) && GetConVarNumber("jet_cockpitview")>0 ) then 
			
		local a = helo:GetAngles()

		if( helo.Gauges ) then 
	
			local ang = helo:GetAngles()  + ( helo.Gauges.AttitudeIndicator.Ang or Angle() )
			ang:RotateAroundAxis( helo:GetRight(), 90 ) 
			ang:RotateAroundAxis( helo:GetForward(), 90 ) 
			
			x,y = size,size	
		
			if( helo.Gauges.AttitudeIndicator ) then 
				local scale = helo.Gauges.AttitudeIndicator.Scale or .01 
				local newAng = ang
				cam.Start3D( )
				cam.Start3D2D( helo:LocalToWorld( helo.Gauges.AttitudeIndicator.Pos ), newAng, scale ) 
					 NeurotecFlightmodules.AttitudeIndicator( size, x, y, a ) 
				cam.End3D2D()
				cam.End3D()
			end 
			
			if( helo.Gauges.Compass ) then 
				local scale = helo.Gauges.Compass.Scale or .01 
				local newAng = ang + ( helo.Gauges.Compass.Ang or Angle() )
				cam.Start3D( )
				cam.Start3D2D( helo:LocalToWorld( helo.Gauges.Compass.Pos ), newAng,  scale )
					NeurotecFlightmodules.Compass( size, x, y, a ) 
				cam.End3D2D()
				cam.End3D()
			end 
		
			if( helo.Gauges.Clock ) then 
				local scale = helo.Gauges.Clock.Scale or .01 
				local newAng = ang + ( helo.Gauges.Clock.Ang or Angle() )
				cam.Start3D( )
				cam.Start3D2D( helo:LocalToWorld( helo.Gauges.Clock.Pos ), newAng,  scale )
					NeurotecFlightmodules.Clock( size, x, y, a ) 
				cam.End3D2D()
				cam.End3D()
			end 
			
			if( helo.Gauges.VSpeed ) then 
				local scale = helo.Gauges.VSpeed.Scale or .01 
				local newAng = ang + ( helo.Gauges.VSpeed.Ang or Angle() )
				cam.Start3D( )
				cam.Start3D2D( helo:LocalToWorld( helo.Gauges.VSpeed.Pos ), newAng,  scale )
					NeurotecFlightmodules.VSpeedIndicator( size, x, y, helo )
				cam.End3D2D()
				cam.End3D()
			end 
			if( helo.Gauges.EngineTemp ) then 
				local scale = helo.Gauges.EngineTemp.Scale or .01 
				local newAng = ang + ( helo.Gauges.EngineTemp.Ang or Angle() )
				cam.Start3D( )
				cam.Start3D2D( helo:LocalToWorld( helo.Gauges.EngineTemp.Pos ), newAng,  scale )
					NeurotecFlightmodules.EngineTemp( size, x, y, helo )
				cam.End3D2D()
				cam.End3D()
			end 
			
			if( helo.Gauges.FuelCounter ) then 
				local scale = helo.Gauges.FuelCounter.Scale or .01 
				local newAng = ang + ( helo.Gauges.FuelCounter.Ang or Angle() )
				cam.Start3D( )
				cam.Start3D2D( helo:LocalToWorld( helo.Gauges.FuelCounter.Pos ), newAng,  scale )
					NeurotecFlightmodules.FuelCounter( size, x, y, helo )
				cam.End3D2D()
				cam.End3D()
			end 
			
			
			if( helo.Gauges.Altimeter ) then  
				local scale = helo.Gauges.Altimeter.Scale or .01 
				local newAng = ang + ( helo.Gauges.Altimeter.Ang or Angle() )
				cam.Start3D( )
				cam.Start3D2D( helo:LocalToWorld( helo.Gauges.Altimeter.Pos ), newAng,  scale )
					NeurotecFlightmodules.Altimeter( size, x, y, helo )
				cam.End3D2D()
				cam.End3D()
			end 
			if( helo.Gauges.Throttle ) then  
				local scale = helo.Gauges.Throttle.Scale or .01 
				-- local desAng = 
				local newAng = ang
				-- newAng:RotateAroundAxis( self:GetRight(), 
				-- + ( helo.Gauges.Throttle.Ang or Angle() )
				cam.Start3D( )
				cam.Start3D2D( helo:LocalToWorld( helo.Gauges.Throttle.Pos ), newAng,  scale )
					NeurotecFlightmodules.Throttle( size, x, y, ply ) 
				cam.End3D2D()
				cam.End3D()
			end 		
			
			if( helo.Gauges.CockpitLight ) then  

				NeurotecFlightmodules.CockpitLight( helo.Gauges.CockpitLight , helo ) 
	
			end 
		
			if( helo.Gauges.Airspeed ) then  
				local scale = helo.Gauges.Airspeed.Scale or .01 
				local newAng = ang + ( helo.Gauges.Airspeed.Ang or Angle() )
				cam.Start3D( )
				cam.Start3D2D( helo:LocalToWorld( helo.Gauges.Airspeed.Pos ), newAng,  scale )
					NeurotecFlightmodules.Airspeed( size, x, y, helo:GetVelocity():Length(), helo.MaxVelocity  )
				cam.End3D2D()
				cam.End3D()
			end 
			if( helo.Gauges.GunCamera ) then 
				local scale = helo.Gauges.GunCamera.Scale or .01 
				-- local newAng = ang + ( helo.Gauges.Airspeed.Ang or Angle() )
				local gun = helo:GetNWEntity("NeuroCrosshairPointer")
				newAng = helo:GetAngles()
				newAng:RotateAroundAxis( helo:GetRight(), 180 ) 
				newAng:RotateAroundAxis( helo:GetForward(), 90 ) 
				newAng:RotateAroundAxis( helo:GetUp(), 90 ) 
				if( IsValid( gun ) ) then 
					-- print("what")
					-- NeurotecFlightmodules.GunCamera( gun:GetPos(), newAng, gun, scale )
...
function JetFighter.MarkEnemies()
	...
	for k,v in pairs( ents.FindByClass( "sent*" ) ) do
		...
		if ( inLOS( JetFighter.Plane, v ) &&  IsValid( v ) && v.PrintName && v != JetFighter.Plane && ( v.Type == "vehicle" || ( v.PrintName == "Missile" ) ) ) then
...
-- */
	...
	for k,v in pairs( player.GetAll() ) do
		...
		if ( v:GetPos():Distance( JetFighter.Plane:GetPos() ) < 10500 && v != JetFighter.Pilot && inLOS( JetFighter.Plane, v ) ) then
			
			if ( v:OnGround() && !v.ColdBlooded ) then
...
local LastPointerPos = Vector(0,0,0)
...
function JetFighter.DrawCrosshair( )
	...
	local pointer = JetFighter.Plane:GetNWEntity("NeuroCrosshairPointer") 
	if( IsValid( pointer ) ) then  
		LastPointerPos = LerpVector( 0.25, LastPointerPos, pointer:GetPos() + pointer:GetForward() * 5000  )
		pos = LastPointerPos:ToScreen() 
	end 
	...
	if( GetConVarNumber("jet_HUD") == 0 ) then
		...
		if( IsValid( pointer ) ) then 
			local p = ( JetFighter.Plane:GetPos( ) + JetFighter.Plane:GetUp( ) * offs + JetFighter.Plane:GetForward( ) * 3000 ):ToScreen()
			local X,Y = p.x,p.y 
			surface.SetDrawColor( 0, 255, 0, 100 )
			surface.DrawCircle( X, Y, 8, Color( 0, 255, 0, 100) ) --horizon circle
			surface.DrawLine( X+8*sinr, Y-8*cosr, X+20*sinr, Y-20*cosr ) --up
...
function JetFighter.OldSchoolCrosshair()
	...
	if( GetConVarNumber("jet_cockpitview")>0 ) then 
		tpos = ( JetFighter.Plane:GetPos() + LocalPlayer():EyeAngles():Forward() * 3500 + JetFighter.Plane:GetUp() * 0 ):ToScreen()
	end 
	local size,l = 16, 8
...
function JetFighter.HUDinstruments(dangerzone)
	...
	//airspeed
	if( JetFighter.Plane.Gauges ) then return end 
	
	local spd = JetFighter.Plane:GetVelocity():Length()
	local maxspd = math.floor( JetFighter.Plane:GetNetworkedInt( "MaxSpeed", 0 ) )
	local throttle = JetFighter.Pilot:GetNWInt("HelicopterThrottle")
	...
	draw.SimpleText( "Manifold Pressure:"..throttle.."%", "TargetID", sposX, sposY+16, Color(255, 255-dangerzone, 255-dangerzone, 200), TEXT_ALIGN_CENTER, TEXT_ALIGN_LEFT )		
...
//throttle indicator
	...
	draw.SimpleText( "Manifold Pressure:"..throttle.."%", "TargetID", size+55, ScrH()-size+18, Color(255, 255-dangerzone, 255-dangerzone, 200), TEXT_ALIGN_CENTER, TEXT_ALIGN_LEFT )		
...
*/	
	...
	local wep = JetFighter.Plane:GetNetworkedString("NeuroPlanes_activeWeapon",tostring("NONE_AVAILABLE")) -- should this really be necessary?
	draw.SimpleText( wep, "TargetID", ScrW() - 200, ScrH()-15, Color( 255, 255, 255, 245 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_LEFT )
...
function DrawPlaneHUD()
	...
	if( JetFighter.Plane && JetFighter.Plane.WingPositions ) then 
	
		JetFighter.AttitudeIndicator()
