...
function ENT:NeuroMicro_SpawnImpactFire( pos, ent  )
   ...
   fire:SetKeyValue("firesize", ( 128 * math.Rand(0.7, 1.1)))
   fire:SetKeyValue("fireattack", 32 )
...
function ENT:PhysicsCollide( data, physobj )
	...
	if( IsValid( data.HitEntity ) && data.HitEntity.HealthVal != nil ) then
		...
		if( math.random(1,2) == 2 ) then 
			...
			-- print("F-F-F-F-F-FLAAAAMING")
			
			-- local fire = self:NeuroMicro_SpawnImpactFire( data.HitPos, data.HitEntity )
			-- print(fire )
			local tempFire = ents.Create("neuro_Fire")
			tempFire:SetPos( data.HitPos )
			tempFire:SetAngles( self:GetAngles() )
			tempFire:Spawn()
			tempFire:PhysicsDestroy()
			tempFire:SetParent( data.HitEntity )
			-- ParticleEffectAttach( "fire_b", PATTACH_ABSORIGIN_FOLLOW, tempFire, 0 ) -- big fire 
   
			if( data.HitEntity.FiremenHealth && data.HitEntity.FiremenHealth > 0 ) then 
				
				tempFire:Fire("kill","",math.random(5,15))
			
			else
				
				tempFire:Fire("kill","",math.random(35,60))
