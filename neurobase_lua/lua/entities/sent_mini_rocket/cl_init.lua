...
function ENT:Initialize()
	...
	local pos = self:GetPos() + self:GetForward() * -6
...
function ENT:Draw()
	...
	if (self.cooltime < CurTime()) then
		...
		local fire = emitter:Add("effects/smoke_a", self:GetPos() + self:GetForward()*-7)
		...
		local fire = emitter:Add("effects/yellowflare", self:GetPos() + self:GetForward()*-7)
		fire:SetVelocity(self:GetForward()*-10)
