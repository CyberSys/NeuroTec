...
function ENT:PhysicsCollide( data, physobj )
	timer.Simple(0,function()
		-- print( physobj:GetMass(), data.HitEntity  )
		...
		if( IsValid( data.HitEntity )  && data.HitEntity != game.GetWorld() ) then
...
function ENT:OnRemove()
	...
	local dmg = 400
	local radius = self.Radius or 72
	...
	-- print( self.Owner, radius, dmg )
	util.BlastDamage( self, self.Owner, self:GetPos() + self:GetForward() * 10, radius, dmg )
