...
function ENT:OnRemove()
	...
	if( self:WaterLevel() > 0 ) then
		...
		-- util.Effect("WaterSurfaceExplosion", impact)
		local wfx = "water_impact_big_mini"
		if( dmg > 1500 ) then 
			wfx = "water_impact_big"
		end 
		
		ParticleEffect( wfx, self:GetPos() + Vector( 0,0,2), Angle( 0,0,0 ), nil )
		
	else
		local pfx = "RT_impact_tank_mini"
		if( dmg > 1500 ) then 
			
			pfx = "fireboom_explosion"
		
		end 
		
		ParticleEffect( pfx, self:GetPos() + Vector( 0,0,2), norm, nil )
