	...
	if( self:GetNetworkedBool("IsStarted", false ) ) then
		...
		for i=1,#pos do
			...
			if ( particle ) then
				...
				local fatness = math.Clamp( self:GetVelocity():Length()/15, 0, 25 )
				
				--particle:SetVelocity( Vector( math.Rand( -1.5,1.5),math.Rand( -1.5,1.5),math.Rand( 2.5,15.5)  ) )
                particle:SetVelocity(  self:GetUp() * 115 )
				particle:SetDieTime( math.Rand( 4, 6 ) )
				particle:SetStartAlpha( math.Rand( 120, 230 ) )
				particle:SetEndAlpha( 0 )
				particle:SetStartSize( math.Rand( 13+fatness, 15+fatness ) )
				particle:SetEndSize( math.Rand( 128+fatness, 136+fatness ) )
