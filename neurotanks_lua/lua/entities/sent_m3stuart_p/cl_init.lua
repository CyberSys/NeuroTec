...
function ENT:Draw()
	...
	if( self:GetNetworkedBool("IsStarted", false ) ) then
		-- Exhaust	
		local pos = { Vector( -45, 36, 50 ), Vector( -45, -36, 50 ) }
		...
		for i =1,2 do
			...
			if ( particle ) then
                ...
                particle:SetVelocity( self:GetForward() * -90 + self:GetRight() * 0 + self:GetUp() * -30 )
				particle:SetDieTime( math.Rand( 2, 3 ) )
				particle:SetStartAlpha( math.Rand( 20, 20 ) )
				particle:SetEndAlpha( 0 )
				particle:SetStartSize( math.Rand( 5+fatness, 10+fatness ) )
