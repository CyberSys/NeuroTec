	...
	if( self:GetNetworkedBool("IsStarted", false ) ) then
		...
		for i=1,#pos do
			...
			if ( particle ) then
                ...
                particle:SetVelocity( self:GetForward() * 1 + self:GetRight() * 1 + self:GetUp() * 55 )
				particle:SetDieTime( math.Rand( 0.3, 1.2 ) )
				particle:SetStartAlpha( math.Rand( 20, 30 ) )
				particle:SetEndAlpha( 0 )
				particle:SetStartSize( math.Rand( 23, 25 ) )
				particle:SetEndSize( math.Rand( 26, 32 ) )
