...
function ENT:Draw()
	...
	if( self:GetNetworkedBool("IsStarted", false ) ) then
		-- Exhaust	
		local pos = { Vector( -1060, -60, 760 ),Vector( -1060, 20, 760 ),Vector( -1250, -60, 760 ),Vector( -1250, 20, 760 ),Vector( 290, 600, 760 ),Vector( 290, -600, 760 ) }
		...
		local vel = math.Clamp( math.floor(self:GetVelocity():Length()), 0, 35 )
			...
			for i=1,#pos do
				...
				if ( particle ) then
					...
					particle:SetDieTime( math.Rand( 0.1, 3.46 ) )
					particle:SetStartAlpha( math.Rand( 30, 70 ) )
					particle:SetEndAlpha( 0 )
					particle:SetStartSize( math.Rand( 50, 100 )+vel )
