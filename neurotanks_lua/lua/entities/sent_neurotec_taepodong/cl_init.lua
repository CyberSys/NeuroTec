...
function ENT:EffectDraw_fire()
	...
	local pos = self:LocalToWorld( Vector( -474, 0, 0 ) ) -- self:GetPos() + self:GetForward() * -200
	...
	for i=1,1 do
		...
		if ( particle ) then
			...
			particle:SetStartAlpha( math.Rand( 85, 125 ) )
			particle:SetEndAlpha( 0 )
			-- particle:EnableCollisions( true )
			particle:SetStartSize( math.Rand( 55, 66 ) )
			particle:SetEndSize( math.Rand( 666, 777 ) )
			particle:SetRoll( math.Rand( 0, 360 ) )
			particle:SetRollDelta( math.Rand( -1, 1 ) )
			particle:SetColor( math.Rand( 245, 255 ), math.Rand( 245, 255 ), math.Rand( 245, 255 ) ) 
			particle:SetAirResistance( 100 ) 
			particle:SetGravity( self:GetForward() * -1500 + VectorRand():GetNormalized()*math.Rand(-140, 140)+Vector(0,0,math.random(-15, 15)) ) 	
