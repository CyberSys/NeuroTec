...
function ENT:Draw()
	...
	if( self:GetNetworkedBool("IsStarted", false ) ) then
		-- Exhaust	
		local pos = Vector( -109, -8, 52 )

		
		local particle = self.Emitter:Add( "particle/smokesprites_000"..math.random(1,9), self:LocalToWorld( pos ) + self:GetRight() * math.random(-32,32) )

		if ( particle ) then
			
			--particle:SetVelocity( Vector( math.Rand( -1.5,1.5),math.Rand( -1.5,1.5),math.Rand( 2.5,15.5)  ) )
			particle:SetVelocity( self:GetForward() * -10.5 + self:GetRight() * 25 + self:GetUp() * 6.5 )
