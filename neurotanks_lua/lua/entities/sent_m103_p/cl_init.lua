...
function ENT:Draw()
	...
	if( self:GetNetworkedBool("IsStarted", false ) ) then
		-- Exhaust	
		local pos = { Vector( -114, 15, 65 ), Vector( -114, -15, 65 ) }
		...
		local vel = math.Clamp( math.floor(self:GetVelocity():Length()), 0, 35 )
			...
			for i=1,#pos do
				...
				if ( particle ) then
					...
					particle:SetDieTime( math.Rand( 0.1, 4.46 ) )
					particle:SetStartAlpha( math.Rand( 8, 8 ) )
					particle:SetEndAlpha( 0 )
					particle:SetStartSize( math.Rand( 5, 60 )+vel )
					particle:SetEndSize( math.Rand( 10, 80 ) )
