...
function ENT:Draw()
	...
	if( self:GetNetworkedBool("IsStarted", false ) ) then
		-- Exhaust pos
		local pos = { Vector( -78, 28, 44 ), Vector( -78, -28, 44 ) }
		...
		for i=1,#pos do
			...
			if ( particle ) then
                ...
                particle:SetVelocity( self:GetForward() * -10.5 + self:GetUp() * 26.5 )
				particle:SetDieTime( math.Rand( 0.51, 1.2 ) )
				particle:SetStartAlpha( math.Rand( 20, 30 ) )
				particle:SetEndAlpha( 0 )
				particle:SetStartSize( math.Rand( 8, 12 ) )
				particle:SetEndSize( math.Rand( 11, 22 ) )
				particle:SetRoll( math.Rand(0, 360) )
				particle:SetRollDelta( math.Rand(-1, 1) )
				particle:SetColor( math.Rand(35,45), math.Rand(35,45), math.Rand(35,45) ) 
				particle:SetAirResistance( 100 ) 
				particle:SetGravity( VectorRand():GetNormalized()*math.Rand(7, 16)+Vector(0,0,math.Rand(170, 210)) ) 	
