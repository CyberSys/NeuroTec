	...
	if( self:GetNetworkedBool("IsStarted", false ) ) then
		...
		local pos = Vector( -77, 33, 42 )
			...
			if ( particle ) then
                ...
                particle:SetVelocity( self:GetForward() * -55.5 + self:GetUp() * 6.5 )
				particle:SetDieTime( math.Rand( 1, 2 ) )
				particle:SetStartAlpha( math.Rand( 20, 30 ) )
				particle:SetEndAlpha( 0 )
				particle:SetStartSize( math.Rand( 9, 12 ) )
				particle:SetEndSize( math.Rand( 15, 19 ) )
