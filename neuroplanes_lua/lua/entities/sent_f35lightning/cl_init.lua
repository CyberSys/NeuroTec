...
function ENT:CalcView( ply, Origin, Angles, Fov )
	...
	if( IsValid( plane ) && ply:GetNetworkedBool( "InFlight", false )  ) then
		...
		if( GetConVarNumber("jet_cockpitview") > 0 ) then
			...
			pos = plane:LocalToWorld( Vector( 197, 0, 32 ) ) //Origin//
		...
		if ( ply:KeyDown( IN_ATTACK ) ) then
				...
				ang.p = ang.p + math.Rand(-.4,.4)
				ang.y = ang.y + math.Rand(-.4,.4)
					
		end
		
		view = {
			origin = pos,
			angles = Angle( ang.p, ang.y, pAng.r / 1.75 ),
			fov = fov
			}
...
function ENT:Draw()
	...
	//self:DrawJetFlames( self:GetPos() + self:GetForward() * -180 + self:GetRight() * 0 + self:GetUp() * 5 )
	
	if ( self:GetVelocity():Length() < 1500 ) then
		
		self:DrawJetFlamesStatic( self:LocalToWorld( Vector( 118, 0, -35 ) ) )
...
function ENT:DrawJetFlames( pos )
	...
	local x = ( self:GetVelocity():Length() / 180 ) + 1.58
	local vOffset = pos + self:GetUp() * -3.0 + self:GetForward() * -x
...
function ENT:DrawJetFlamesStatic( pos )

	local vOffset = pos + self:GetUp() * -1
	local vNormal = ( vOffset - pos ):GetNormalized()
	local scroll = CurTime() * -20
	local Scale = 2.0
