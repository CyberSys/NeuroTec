...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.Target == self.Owner ) then
		...
		if( IsValid( a ) ) then
			...
			if ( a.HealthVal || a:IsNPC() || a:IsPlayer() ) then
			
				self.Target = a
				self.Owner.LastAttacker = a
				
			elseif( IsValid( a.Owner ) ) then
				
				self.Target = a.Owner
				self.Owner.LastAttacker = a.Owner
	...
	if ( self.HealthVal < 300 && !self.Burning ) then
		
		self.Owner:EmitSound("lockon/voices/IamHit.mp3",511,100)
		
		if( IsValid( self.Owner ) && IsValid( self.Owner.Pilot ) ) then
			
			self.Owner.Pilot:PrintMessage( HUD_PRINTCHAT, "AC-130: Your escort is taking heavy damage!" )
		...
		pos[1] = p + f * -180 + r * -80 + u * 17
		pos[2] = p + f * -180 + r * 80 + u * 17
...
function ENT:PhysicsCollide( data, physobj )
	...
	if( IsValid( data.HitEntity ) && data.HitEntity:GetClass() == self:GetClass() ) then return end
...
function ENT:Think()
	...
	if ( self.Destroyed ) then 
		...
		if self.DeathTimer > 40 && self:GetVelocity():Length() > 10 then
	...
	-- print( self.Target, self.Target:GetClass() )
	...
	if( IsValid( self.Target ) ) then
		...
		if( self.Target:OnGround() && logic1 && logic2 && self.Target != self.Owner ) then // Fly in, shoot, leave.
			...
			self:EmitSound("lockon/voices/engagingbandit.mp3",511,100)
			self:DoPrimaryAttack()
			
		elseif( self.Target:OnGround() && !logic1 || trace.HitWorld ) then
		
			local p = self.Target:GetPos() + self.Target:GetUp() * ( 4048 * math.sin(CurTime() ) )
		...
		elseif ( !self.Target:OnGround() && !trace.HitWorld ) then
			...
			if( self.Target == self.Owner ) then
				...
				self.PavePenny:SetPos( self.CycleOffset )
		...
		local smallcone = ( self.Yaw > -6 && self.Yaw < 6 && self.Pitch > -6 && self.Pitch < 6 && self.TargetDistance < 9001 )
		...
		if ( IsValid( wep ) ) then
			...
			if ( wep.LastAttack + wep.Cooldown <= CurTime() ) then
				...
				if ( logic && IsValid( self.Target ) && self.Target != self.Owner && wep.Type == "Homing" ) then
					...
					self:EmitSound("lockon/voices/missileAway.mp3",511,100)
					
					if( self.Target:OnGround() && wep.Type == "Bomb" ) then
...
function ENT:PrimaryAttack()
 	...
 	bullet.Callback    = function ( a, b, c )
							...
							util.BlastDamage( self.Pilot, self.Pilot, b.HitPos, 250, math.random(18,45) )
...
function ENT:PhysicsSimulate( phys, deltatime )
	...
	if ( IsValid( self.Pilot ) && IsValid( self.Target ) ) then
		...
		local fpos = self:FindAvoidancePoint( self:GetPos(), self:GetForward(), 10000 )
