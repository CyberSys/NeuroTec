...
ENT.PrintName	= "F-4E Phantom II"
ENT.Model = "models/hawx/planes/f-4e phantom ii.mdl"
//Speed Limits
ENT.MaxVelocity = 1.8 * 2370
ENT.MinVelocity = 0
...
ENT.MachineGunOffset = Vector( 363, 0, 43 )
ENT.CrosshairOffset = 43

ENT.NumRockets = nil
ENT.PrimaryCooldown = 0.12


function ENT:SpawnFunction( ply, tr)

	local SpawnPos = tr.HitPos + tr.HitNormal * 100
	local ent = ents.Create( "sent_f-4e_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	...
	*/
	...
	self.Armament = {
					...
					{ 
						...
						Pos = Vector( -25, -145, 41), 							// Pos, Hard point location on the plane fuselage.
					...
					{ 
						...
						Pos = Vector( 82, -85, 34 ), 							// Pos, Hard point location on the plane fuselage.
						Ang = Angle( 0, 0, 0), 									// Ang, object angle
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -144, -244, 80 )
	self.TrailPos[2] = Vector( -144, 244, 80 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -177 + self:GetRight() * -24 + self:GetUp() * 69
		p[2] = self:GetPos() + self:GetForward() * -177 + self:GetRight() * 24 + self:GetUp() * 69
		for _i=1,2 do
