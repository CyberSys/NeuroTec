...
ENT.PrintName	= "X-02 Wyvern"
ENT.Model = "models/hawx/planes/x-02 wyvern.mdl"
//Speed Limits
ENT.MaxVelocity = 2000
ENT.MinVelocity = 1100
...
ENT.MachineGunOffset = Vector( 92, 68, 4 )
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_x02wyvern" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	...
	*/
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -113, -354, 13 )
	self.TrailPos[2] = Vector( -113, 354, 13 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -333 + self:GetRight() * 40 + self:GetUp() * 24
		p[2] = self:GetPos() + self:GetForward() * -333 + self:GetRight() * -40 + self:GetUp() * 24
		for _i=1,2 do
...
---- "Collision Sound" if the spitfire collide with something and crash if it fly over 70% of the maximum speed
	if ( self.Speed > self.MaxVelocity * 0.25 && data.DeltaTime > 0.2 ) then  
		self:EmitSound( "lockon/PlaneHit.mp3", 510, math.random( 100, 130 ) )
			if ( self:GetVelocity():Length() > self.MaxVelocity * 0.45 ) then
				self:DeathFX()
			end
	end
