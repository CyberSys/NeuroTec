...
ENT.PrintName = "Dassault Rafale"
ENT.Model = "models/hawx/air/dassault rafale m.mdl"
//Speed Limits
...
ENT.MachineGunOffset = Vector( 20, 80, 45 )
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_rafale_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	
	if( string.find(string.lower(ply:GetName()),"hoffa") != nil ) then
		
		ent:SetSkin( math.random( 4, 5 ) )
	...
	*/
	...
	self.Armament = {
					...
					{ 
						...
						Pos = Vector( -149, -212, 59), 							// Pos, Hard point location on the plane fuselage.
					...
					/* Exocet AM-39 - Centered */
					{ 
						PrintName = "Exocet AM-39", 
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( 28, -102, 94 )
	self.TrailPos[2] = Vector( 28, 102, 94 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -230 + self:GetRight() * -27 + self:GetUp() * 75
		p[2] = self:GetPos() + self:GetForward() * -230 + self:GetRight() * 27 + self:GetUp() * 75
		for _i=1,2 do
