...
function ENT:CalcView( ply, Origin, Angles, Fov )
	...
	if( IsValid( plane ) && ply:GetNetworkedBool( "InFlight", false )  ) then
		...
		if( GetConVarNumber("jet_cockpitview") > 0 ) then
			...
			pos = plane:LocalToWorld( Vector( 120, 0, 96 ) ) //Origin//
...
function ENT:Draw()
	...
	local pos = {}
	pos[1] = self:GetPos() + self:GetForward() * -260 + self:GetRight() * -20 + self:GetUp() * 75
	pos[2] = self:GetPos() + self:GetForward() * -260 + self:GetRight() * 20 + self:GetUp() * 75	
