...
ENT.PrintName	= "A-12 Avenger II"
ENT.Model = "models/hawx/planes/a-12 avenger ii.mdl"
//Speed Limits
ENT.MaxVelocity = 1.8 * 930
ENT.MinVelocity = 0
...
ENT.MachineGunOffset = Vector( 251, -2, 50 )
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_a-12_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	...
	*/
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -221, -413, 89 )
	self.TrailPos[2] = Vector( -221, 413, 89 )
	...
	self:SpawnPassengerSeat( Vector( 62, 0, 97 ),  Angle( 0, -90, 0 ) )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -156 + self:GetRight() * -10 + self:GetUp() * 73
		p[2] = self:GetPos() + self:GetForward() * -156 + self:GetRight() * 10 + self:GetUp() * 73
		p[3] = self:GetPos() + self:GetForward() * -156 + self:GetRight() * -29 + self:GetUp() * 73
		p[4] = self:GetPos() + self:GetForward() * -156 + self:GetRight() * 29 + self:GetUp() * 73
...
function ENT:Use(ply,caller)
	...
	if ( !self.IsFlying && !IsValid( self.Pilot ) ) then 
		...
		self.PilotModel = self:SpawnPilotModel( Vector( 136, 0, 100 ), Angle( -5, 0, 0 ) )
