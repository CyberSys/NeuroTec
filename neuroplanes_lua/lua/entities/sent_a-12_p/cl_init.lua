...
function ENT:CalcView( ply, Origin, Angles, Fov )
	...
	if( IsValid( plane ) && ply:GetNetworkedBool( "InFlight", false )  ) then
		...
		if( GetConVarNumber("jet_cockpitview") > 0 ) then
			...
			pos = plane:LocalToWorld( Vector( 140, 0, 122 ) ) //Origin//
...
function ENT:Draw()
	...
	if( self:GetNetworkedInt( "health" , 0 ) > 100 ) then
		
		self:DrawJetFlames( self:GetPos() + self:GetForward() * -156 + self:GetRight() * -10 + self:GetUp() * 73 )
		self:DrawJetFlames( self:GetPos() + self:GetForward() * -156 + self:GetRight() * 10 + self:GetUp() * 73 )
		self:DrawJetFlames( self:GetPos() + self:GetForward() * -156 + self:GetRight() * -29 + self:GetUp() * 73 )
		self:DrawJetFlames( self:GetPos() + self:GetForward() * -156 + self:GetRight() * 29 + self:GetUp() * 73 )
