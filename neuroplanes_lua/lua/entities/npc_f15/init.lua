...
function ENT:Initialize()
    ...
    self:SetModel( "models/hawx/air/f-15 active.mdl" )		
...
function ENT:Think()
	...
	self.Pitch = math.Clamp(math.floor(self.PhysObj:GetVelocity():Length()/5.3 + 50),0,255)
	for i=1,3 do
	...
	/*if ( !self.Target:OnGround() ) then
		...
		if  self.Entity:GetAngles():Forward():Dot((self.Target:GetPos() - self.Entity:GetPos()):GetNormalized()) > 0.4 && self:GetPos():Distance(self.Target:GetPos()) < 3200 then
	...
	if( self.mYaw > -2 || self.mYaw < 2 ) then
	...
	elseif ( self.mYaw > -self.YawMax && self.mYaw < self.YawMax ) then
	
		self.YawSpeed = self.YawSpeed / 3 // Compensate for too high momentum when infront.
	
	end
	
	self.PhysObj:SetAngle( self.mAng2 + Angle( self.RollVal2, self.YawSpeed, r ) )
