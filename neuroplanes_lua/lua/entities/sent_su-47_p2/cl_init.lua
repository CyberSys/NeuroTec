...
function ENT:CalcView( ply, Origin, Angles, Fov )
	...
	if( ply:GetNetworkedBool("InFlight", false ) && GetConVarNumber("jet_cockpitview") == 0 ) then
		
		local newpos = plane:GetPos() + plane:GetForward() * 135 + plane:GetUp() * 95
		local pos = LerpVector( 0.015, Origin, newpos )
		
		local ang = ( newpos - plane:GetPos() ):Angle()
	...
	-- local plane = self
	-- local MaxVel = 	plane:GetNetworkedInt( "MaxSpeed", 0 )
