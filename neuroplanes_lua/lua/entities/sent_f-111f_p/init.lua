...
ENT.PrintName	= "F-111F Aardvark"
ENT.Model = "models/hawx/planes/f-111f aardvark.mdl"
//Speed Limits
ENT.MaxVelocity = 1.8 * 2655 // 1650 at low alt
ENT.MinVelocity = 0
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_f-111f_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	...
	*/
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -382, -373, 104 )
	self.TrailPos[2] = Vector( -382, 373, 104 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -560 + self:GetRight() * 36 + self:GetUp() * 83 
		p[2] = self:GetPos() + self:GetForward() * -560 + self:GetRight() * -36 + self:GetUp() * 83 
