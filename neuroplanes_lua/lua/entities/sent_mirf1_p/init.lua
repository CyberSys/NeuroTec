...
ENT.PrintName	= "Dassault Mirage F1"
ENT.Model = "models/hawx/planes/dassault mirage f1.mdl"
//Speed Limits
...
ENT.MachineGunOffset = Vector( 88, -24, 64 )
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_mirf1_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	...
	*/
	...
	self.Armament = {
					...
					{ 
						...
						Pos = Vector( -113, 110, 77), 							// Pos, Hard point location on the plane fuselage.
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -170, -159, 84 )
	self.TrailPos[2] = Vector( -170, 159, 84 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -247 + self:GetUp() * 83
		p[2] = self:GetPos() + self:GetForward() * -247 + self:GetUp() * 83
