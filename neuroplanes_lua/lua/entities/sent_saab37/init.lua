...
ENT.PrintName = "Saab-Viggen"
ENT.Model = "models/hawx/planes/saab-37 viggen.mdl"
//Speed Limits
...
ENT.MachineGunOffset = Vector( 140, 44, 90 )

ENT.CrosshairOffset = 90

ENT.NumRockets = nil
ENT.PrimaryCooldown = 0.105

// Afterburner
ENT.HasPostCombustion = true
ENT.ThrusterPos = {}
ENT.ThrusterPos[1] = Vector( -298, 0, 93 )

function ENT:SpawnFunction( ply, tr)

	local SpawnPos = tr.HitPos + tr.HitNormal * 100
	local ent = ents.Create( "sent_saab37" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	...
	*/
	...
	self.Armament = {
					...
					{ 
						...
						Pos = Vector( -76, -45, -40 ), 							// Pos, Hard point location on the plane fuselage.
						Ang = Angle( 0, 0, 0), 									// Ang, object angle
						Type = "Homing", 										// Type, used when creating the object
						Cooldown = 7.5, 											// Cooldown between weapons
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -150, -268, 67 )
	self.TrailPos[2] = Vector( -150, 268, 67 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * self.ThrusterPos[1].x + self:GetRight() * self.ThrusterPos[1].y + self:GetUp() * self.ThrusterPos[1].z 
		p[2] = self:GetPos() + self:GetForward() * self.ThrusterPos[2].x + self:GetRight() * self.ThrusterPos[2].y + self:GetUp() * self.ThrusterPos[2].z 
		for _i=1,2 do
...
function ENT:PhysicsSimulate( phys, deltatime )
	...
	self:GetPhysicsObject():AddAngleVelocity( Vector( 10, 0, 0 ) )
