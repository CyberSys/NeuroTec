...
ENT.PrintName	= "F/A-18 RC"
ENT.Model = "models/hawx/planes/fa-18 rc.mdl"
//Speed Limits
...
ENT.MachineGunOffset = Vector( 297, 0, 93 )
ENT.CrosshairOffset = 93
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_fa-18rc_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	...
	*/
	...
	self.Armament = {
					...
					{ 
						PrintName = "AIM-132 ASRAAM - SWARM", 
						Mdl = "models/hawx/weapons/aim-132 asraam.mdl",	 
						Pos = Vector( -116, -270, 86 ), 
						Ang = Angle( 0, 0, 0), 
						Type = "Swarm",
						Cooldown = 25,
						--isFirst	= true,
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -169, -262, 84 )
	self.TrailPos[2] = Vector( -169, 262, 84 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -306 + self:GetRight() * -24 + self:GetUp() * 89
		p[2] = self:GetPos() + self:GetForward() * -306 + self:GetRight() * 24 + self:GetUp() * 89
		for _i=1,2 do
