...
ENT.PrintName	= "Su-37 Terminator"
ENT.Model = "models/hawx/planes/su-37 terminator.mdl"
//Speed Limits
ENT.MaxVelocity = 1.8 * 2500
ENT.MinVelocity = 0
...
ENT.MachineGunOffset = Vector( -186, 36, 103 )
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_su-37_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	...
	*/
	...
	self.Armament = {
					...
					{ 
						...
						Pos = Vector( -446, -281, 92), 							// Pos, Hard point location on the plane fuselage.
					...
					{ 
						PrintName = "AIM-07 Sparrow"		,		 					// print name, used by the interface
						Mdl = "models/hawx/weapons/aim-07 sparrow.mdl",  				// model, used when creating the object
						Pos = Vector( -461, -237, 78), 							// Pos, Hard point location on the plane fuselage.
						Ang = Angle( 0, 0, 0), 									// Ang, object angle
						Type = "Homing", 										// Type, used when creating the object
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -536, -276, 92 )
	self.TrailPos[2] = Vector( -536, 276, 92 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -592 + self:GetRight() * -46 + self:GetUp() * 71
		p[2] = self:GetPos() + self:GetForward() * -592 + self:GetRight() * 46 + self:GetUp() * 71
		for _i=1,2 do
