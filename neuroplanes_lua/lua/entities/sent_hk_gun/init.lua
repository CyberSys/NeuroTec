...
ENT.Model = "models/inaki/props_vehicles/terminator_aerialhk_turret.mdl"
ENT.MaxDamage = 10
ENT.MinDamage = 5
ENT.BlastRadius = 75
...
local Flare = ents.Create("env_flare")
	Flare:SetPos(self:GetPos()+self:GetForward() * 55 )
...
function ENT:Think()
	...
	local entities = ents.FindInSphere(selfpos,2800)
	
	for k,v in pairs(entities) do
	// loooooooooooooooong....
		if (v != self.Entity && v:GetClass() != self.Owner:GetClass()) then
		// .....loooogiiic is...
			if (v:GetClass() == "rpg_missile" || v:IsNPC() || v:IsPlayer() && v:GetPhysicsObject():GetVelocity():Length() > 0) && self.ShouldAttack then
			// looooooong
