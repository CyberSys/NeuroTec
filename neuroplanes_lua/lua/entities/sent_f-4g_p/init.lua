...
ENT.PrintName	= "F-4G Advanced Wild Weasel"
ENT.Model = "models/hawx/planes/f-4g advancedwildweasel.mdl"
//Speed Limits
ENT.MaxVelocity = 1.8 * 2380
ENT.MinVelocity = 0
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_f-4g_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	...
	*/
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -415, -244, 80 )
	self.TrailPos[2] = Vector( -415, 244, 80 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -446 + self:GetRight() * -24 + self:GetUp() * 69
		p[2] = self:GetPos() + self:GetForward() * -446 + self:GetRight() * 24 + self:GetUp() * 69
