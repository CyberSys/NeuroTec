...
ENT.PrintName = "F-15C Eagle"
ENT.Model = "models/hawx/air/f-15c eagle.mdl"
//Speed Limits
ENT.MaxVelocity = 3500
ENT.MinVelocity = 0

// How much the plane will rotate around the Z axis when turning. Lower Value = More Angle. Recomended = 2.0-2.5 for super sonic jets, 3.5 to 5.5 for larger stuff.
ENT.BankingFactor = 1.2

ENT.InitialHealth = 2200
ENT.HealthVal = nil
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_f-15c_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	
	if( string.find(string.lower(ply:GetName()),"starchick") != nil ) then
	...
	*/
	...
	self.Armament = {
					...
					{ 
						...
						Pos = Vector( -29, -126, 68), 							// Pos, Hard point location on the plane fuselage.
					...
					/* Drop Tank - Right Wing */
					{ 
						PrintName = "Drop Tank", 
						Mdl = "models/hawx/weapons/drop tank.mdl",	 
						Pos = Vector( -47, -111, 66 ), 
						Ang = Angle( 0, 0, 0), 
						Type = "Effect",
						Cooldown = 0,
						isFirst	= nil,
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -131, -245, 77 )
	self.TrailPos[2] = Vector( -131, 245, 77 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -240 + self:GetRight() * -35 + self:GetUp() * 69
		p[2] = self:GetPos() + self:GetForward() * -240 + self:GetRight() * 35 + self:GetUp() * 69
		for _i=1,2 do
...
function ENT:PrimaryAttack()
	...
	if ( !IsValid( self.Pilot ) ) then
 	...
 	local bullet = {} 
 	bullet.Num 		= 1
 	bullet.Src 		= self.Weapon:GetPos() + self.Weapon:GetForward() * 161
