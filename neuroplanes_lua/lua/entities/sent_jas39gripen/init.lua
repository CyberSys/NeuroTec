...
ENT.PrintName	= "Saab JAS-39 Gripen"
ENT.Model = "models/hawx/planes/saab-39 gripen.mdl"
//Speed Limits
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_jas39gripen" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -409 + self:GetRight() * 0 + self:GetUp() * 74
		p[2] = self:GetPos() + self:GetForward() * -409 + self:GetRight() * 0 + self:GetUp() * 74
