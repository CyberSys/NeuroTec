...
ENT.PrintName	= "Su-35 SuperFlanker"
ENT.Model = "models/hawx/planes/su-35 superflanker.mdl"
//Speed Limits
...
ENT.MachineGunOffset = Vector( 115, 36, 103 )
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_su-35_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	...
	*/
	...
	self.Armament = {
					...
					{ 
						...
						Pos = Vector( -147, -281, 92), 							// Pos, Hard point location on the plane fuselage.
					...
					{ 
						...
						Pos = Vector( -170, -236, 77), 							// Pos, Hard point location on the plane fuselage.
						Ang = Angle( 0, 0, 0), 									// Ang, object angle
						Type = "Homing", 										// Type, used when creating the object
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -237, -276, 92 )
	self.TrailPos[2] = Vector( -237, 276, 92 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -305 + self:GetRight() * -46 + self:GetUp() * 71
		p[2] = self:GetPos() + self:GetForward() * -305 + self:GetRight() * 46 + self:GetUp() * 71
		for _i=1,2 do
