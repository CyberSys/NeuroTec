...
ENT.PrintName	= "SR-71 blackbrid"
ENT.Model = "models/hawx/planes/sr-71 blackbird.mdl"
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_sr-71_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	...
	self.Armament = {
					{ 
						...
						Pos = Vector( -5, 0, 0), 							// Pos, Hard point location on the plane fuselage.
					...
					{ 
						PrintName = "AGM-65 Tesla Maverick", 
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -474, -327, 75 )
	self.TrailPos[2] = Vector( -474, 327, 75 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -443 + self:GetRight() * -166 + self:GetUp() * 90
		p[2] = self:GetPos() + self:GetForward() * -443 + self:GetRight() * 166 + self:GetUp() * 90	
