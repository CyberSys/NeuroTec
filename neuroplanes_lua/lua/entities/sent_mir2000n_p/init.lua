...
ENT.PrintName	= "Dassault Mirage 2000 N"
ENT.Model = "models/hawx/planes/dassault mirage 2000 n.mdl"
//Speed Limits
ENT.MaxVelocity = 1.8 * 2338
ENT.MinVelocity = 0
...
ENT.MachineGunOffset = Vector( 185, 0, 76 )
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_mir2000n_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	...
	*/
	...
	self.Armament = {
					...
					{ 
						PrintName = "AIM-132 Tesla ASRAAM", 
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -194, -181, 49 )
	self.TrailPos[2] = Vector( -194, 181, 49 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -247 + self:GetRight() * 0 + self:GetUp() * 80
		p[2] = self:GetPos() + self:GetForward() * -247 + self:GetRight() * 0 + self:GetUp() * 80
		for _i=1,2 do
...
function ENT:PrimaryAttack()
	...
	if ( !IsValid( self.Pilot ) ) then
 	...
 	local bullet = {} 
 	bullet.Num 		= 1
 	bullet.Src 		= self.Weapon:GetPos() + self.Weapon:GetForward() * 70
