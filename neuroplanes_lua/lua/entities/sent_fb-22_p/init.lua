...
ENT.PrintName	= "FB-22 StrikeRaptor"
ENT.Model = "models/hawx/planes/fb-22 strikeraptor.mdl"
//Speed Limits
ENT.MaxVelocity = 1.8 * 1224 * 1.92
ENT.MinVelocity = 0
...
// Equipment
ENT.MachineGunModel = "models/airboatgun.mdl" //No miniguns for FB-22!
ENT.MachineGunOffset = Vector( 397, -2, 75 )
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_fb-22_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	...
	*/
	...
	self.Armament = {
					...
					{ 
						...
						Pos = Vector( 84, -49, 60 ), 							// Pos, Hard point location on the plane fuselage.
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -109, -292, 78 )
	self.TrailPos[2] = Vector( -109, 292, 78 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -203 + self:GetRight() * -24 + self:GetUp() * 76
		p[2] = self:GetPos() + self:GetForward() * -203 + self:GetRight() * 24 + self:GetUp() * 76
