...
function ENT:CalcView( ply, Origin, Angles, Fov )
	...
	if( IsValid( plane ) && ply:GetNetworkedBool( "InFlight", false )  ) then
		...
		if( GetConVarNumber("jet_cockpitview") > 0 ) then
			...
			pos = plane:LocalToWorld( Vector( 119, 0, 122 ) ) //Origin//
...
function ENT:Draw()
	...
	if ( p:GetNetworkedBool( "InFlight", false ) && p:GetNetworkedEntity( "Plane", p ) == self  ) then
		...
		local pos = {}
		pos[1] = self:GetPos() + self:GetForward() * -250 + self:GetRight() * -36 + self:GetUp() * 64 
		pos[2] = self:GetPos() + self:GetForward() * -250 + self:GetRight() * 36 + self:GetUp() * 64
...
function ENT:DrawJetFlames( pos )
	...
	render.DrawSprite( pos, 32 + extra, 32 + extra, Color( 255,250,10,165 ) )
