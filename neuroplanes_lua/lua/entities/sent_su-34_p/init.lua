...
ENT.PrintName	= "Su-34 Fullback"
ENT.Model = "models/hawx/planes/su-34 fullback.mdl"
//Speed Limits
ENT.MaxVelocity = 1.8 * 1900
ENT.MinVelocity = 0
...
ENT.MachineGunOffset = Vector( -200, 45, 106 )
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_su-34_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	...
	*/
	...
	self.Armament = {
					...
					{ 
						...
						Pos = Vector( -474, -273, 97), 							// Pos, Hard point location on the plane fuselage.
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -568, -268, 97 )
	self.TrailPos[2] = Vector( -568, 268, 97 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -604 + self:GetRight() * -46 + self:GetUp() * 79
		p[2] = self:GetPos() + self:GetForward() * -604 + self:GetRight() * 46 + self:GetUp() * 79
		for _i=1,2 do
