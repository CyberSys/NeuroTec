...
ENT.MachineGunOffset = Vector( 69, -44, 91 )
ENT.CrosshairOffset = 91

ENT.NumRockets = nil
ENT.PrimaryCooldown = 0.032


function ENT:SpawnFunction( ply, tr)

	local SpawnPos = tr.HitPos + tr.HitNormal * 100
	local ent = ents.Create( "sent_a-6_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	...
	*/
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -156, -311, 85 )
	self.TrailPos[2] = Vector( -156, 311, 85 )
	...
	self:SpawnPassengerSeat( Vector( 87, -15, 80 ), Angle( 0, -90, 0 ) )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -121 + self:GetRight() * -40 + self:GetUp() * 70 
		p[2] = self:GetPos() + self:GetForward() * -121 + self:GetRight() * 40 + self:GetUp() * 70 
		for _i=1,2 do
...
function ENT:PhysicsCollide( data, physobj )
	...
	if ( self.Speed > self.MaxVelocity * 0.2 && data.DeltaTime > 0.2 ) then 
...
function ENT:Use(ply,caller)
	...
	if ( !self.IsFlying && !IsValid( self.Pilot ) ) then 
		...
		self.PilotModel = self:SpawnPilotModel( Vector( 90, 15, 86 ), Angle( 0, 0, 0 ) )
	
		if( IsValid( self.PilotModel ) ) then
			
			self.Pilot:SetNetworkedEntity("NeuroPlanes_Pilotmodel", self.PilotModel )
		...
		if( !IsValid( d ) ) then
			...
			self.Pilot:PrintMessage( HUD_PRINTCENTER, d:Name().." is your new Co-Pilot" )
...
function ENT:PrimaryAttack()
	...
	if ( !IsValid( self.Pilot ) ) then
 	...
 	local bullet = {} 
 	bullet.Num 		= 1
 	bullet.Src 		= self.Weapon:GetPos() + self.Weapon:GetForward() * 179
 	bullet.Dir 		= self.Weapon:GetAngles():Forward()		// Dir of bullet 
 	bullet.Spread 	= Vector( .011, .011, .011 )				// Aim Cone 
 	bullet.Tracer	= math.random( 1, 4 )					// Show a tracer on every x bullets  
 	bullet.Force	= 1050					 					// Amount of force to give to phys objects 
 	bullet.Damage	= 0
 	bullet.AmmoType = "Ar2" 
