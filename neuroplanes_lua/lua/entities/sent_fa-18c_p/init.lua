...
ENT.PrintName	= "FA-18C Hornet"
ENT.Model = "models/hawx/planes/fa-18c hornet.mdl"
//Speed Limits
ENT.MaxVelocity = 1.8 * 1915
ENT.MinVelocity = 0
...
ENT.MachineGunOffset = Vector( 126, 15, 96 )
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_fa-18c_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	...
	*/
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -321, -262, 87 )
	self.TrailPos[2] = Vector( -321, 262, 87 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -447 + self:GetRight() * -21 + self:GetUp() * 91
		p[2] = self:GetPos() + self:GetForward() * -447 + self:GetRight() * 21 + self:GetUp() * 91
		for _i=1,2 do
