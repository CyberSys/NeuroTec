...
ENT.PrintName	= "Mil Mi-28 Havoc"
ENT.Model = "models/bf2/helicopters/mil mi-28/mil mi-28.mdl"
//Speed Limits
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_mi-28_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	...
	*/
	...
	self.Armament = {
					{ 
						...
						Pos = Vector( -33, 90, -41 ), 							// Pos, Hard point location on the plane fuselage.
					...
					{ 
						...
						Pos = Vector( -35, 149, -33), 							// Pos, Hard point location on the plane fuselage.
						Ang = Angle( 0, 0, 0), 									// Ang, object angle
					...
					{ 
						PrintName = "AGM-88 HARM"		,		 			// print name, used by the interface
						Mdl = "models/hawx/weapons/agm-88 harm.mdl",  		// model, used when creating the object
						Pos = Vector( -35, -149, -33), 							// Pos, Hard point location on the plane fuselage.
						Ang = Angle( 0, 0, 0), 									// Ang, object angle
					...
					{ 
						...
						Pos = Vector( -35, -149, -19), 							// Pos, Hard point location on the plane fuselage.
						Ang = Angle( 0, 0, 0), 									// Ang, object angle
						Type = "Homing", 										// Type, used when creating the object
						Cooldown = 10, 											// Cooldown between weapons
						isFirst	= nil,											// If a plane got 2 rockets of the same type, set the first rocket to isFirst = true.
					...
					{ 
						...
						PrintName = "Eryx \"Jericho\""		,		 		// print name, used by the interface
						Mdl = "models/bf2/weapons/eryx/eryx_rocket.mdl",  				// model, used when creating the object
						Pos = Vector( -35, 149, -19), 							// Pos, Hard point location on the plane fuselage.
						Ang = Angle( 0, 0, 0), 								// Ang, object angle
						Type = "Homing", 										// Type, used when creating the object
						Cooldown = 30, 											// Cooldown between weapons
						isFirst	= nil,											// If a plane got 2 rockets of the same type, set the first rocket to isFirst = true.
						Class = "sent_a2a_jericho"								// the object that will be created.
					}; 
					{
						PrintName = "AN ALQ-184",
...
--	self.LoopSound = CreateSound(self.Entity,Sound("bf2/AH1_start_idle_stop.wav"))
	...
	self.ChopperGun:SetModel( "models/bf2/helicopters/wz-10/wz-10_cannon.mdl" )
	self.ChopperGun:SetPos( self:GetPos() + self:GetForward() * 155 + self:GetUp() * -65  )
	...
	self.RotorPropellerPos = ( self:GetUp() * 63 )
	self.TailPropellerPos = ( self:GetForward() * -544 ) + ( self:GetRight() * -26 ) + ( self:GetUp() * 104 )
	...
	self.RotorPropeller:SetModel( "models/bf2/helicopters/mil mi-28/mil mi-28_rotor.mdl" )
	
	self.RotorPhys = self.RotorPropeller:GetPhysicsObject()	
	
	local wheelpos = {}
	wheelpos[1] = Vector( 59, -63, -98 )
	wheelpos[2] = Vector( 59, 63, -98 )
	wheelpos[3] = Vector( -520, 0, -34 )
	...
	self.Wheels[3]:SetPos( self:LocalToWorld( wheelpos[3] ) )
	...
	self.TailPropeller:SetModel( "models/bf2/helicopters/mil mi-28/mil mi-28_tail.mdl" )
	...
	self.Turret:SetModel( "models/bf2/helicopters/ah-1 cobra/ah-1 cobra_turret.mdl" )
	self.Turret:SetPos( self:GetPos() + self:GetForward() * 162 + self:GetUp() * -65  )
	...
	self.PassengerSeat:SetPos( self:LocalToWorld( Vector( 149, 0, -19 ) ) )
	...
	self.WheelWelds[1] = constraint.Axis( self.Wheels[1], self, 0, 0, Vector(0,1,0) , wheelpos[1], 0, 0, 1, 0 )
	self.WheelWelds[2] = constraint.Axis( self.Wheels[2], self, 0, 0, Vector(0,1,0) , wheelpos[2], 0, 0, 1, 0 )
	self.WheelWelds[3] = constraint.Axis( self.Wheels[3], self, 0, 0, Vector(0,1,0) , wheelpos[3], 0, 0, 1, 0 )
	self.WheelPhys = {}
...
function ENT:EjectPilotSpecial()
	...
	self.Pilot:SetPos( self:LocalToWorld( Vector( 180, 100, -40 ) ) )
...
function ENT:FuselageAttack()
	...
	local bullet = {} 
 	...
 	bullet.Damage	= math.random( 15, 55 )
 	bullet.AmmoType = "Ar2" 
...
function ENT:ChopperGunCallback( a, b, c )
	...
	if ( IsValid( self.Pilot ) ) then
		...
		util.BlastDamage( self.Pilot, self.Pilot, b.HitPos, 200, math.random(25,70) )
