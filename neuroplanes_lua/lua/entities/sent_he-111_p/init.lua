...
ENT.Model = "models/wwii/heinkel he-111.mdl"
--Speed Limits
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_he-111_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	...
	self.Armament = {
						...
						{ 
							...
							Pos = Vector( 42, -18, -42), 							-- Pos, Hard point location on the plane fuselage.
						...
						{ 
							PrintName = "Napalm",
							Mdl = "models/props_phx/ww2bomb.mdl" ,
							Pos = Vector( 42, 27, -42), 							-- Pos, Hard point location on the plane fuselage.
							Ang = Angle( 0, 0, 0), 								-- Ang, object angle
							Type = "Bomb", 										-- Type, used when creating the object
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -39, 410, 43 )
	self.TrailPos[2] = Vector( -39, -410, 43 )
	...
	local PropellerPos = { Vector( 146, 100, 0), Vector( 149, -88, 0)}--, Vector( 185, 254, 18), Vector( 185, -254, 18) }
	self.Propeller = {}
	for i = 1, #PropellerPos do		
		self.Propeller[i] = ents.Create("prop_physics")
		self.Propeller[i]:SetPos( self:LocalToWorld( PropellerPos[i] ) )
		self.Propeller[i]:SetModel( "models/wwii/heinkel he-111_propeller.mdl" )
		self.Propeller[i]:SetAngles( self:GetAngles() )
		self.Propeller[i]:Spawn()
	end

	local wheelpos = {}
	wheelpos[1] = Vector( 45.5, 92, -76 )
	wheelpos[2] = Vector( 45.5, -92, -76 )
	wheelpos[3] = Vector( 45.5, 92, -76 )
	wheelpos[4] = Vector( 45.5, -92, -76 )
--	wheelpos[5] = Vector( -352, -3, -10 )
	wheelpos[5] = Vector( -368, -3, 1 )
	wheelpos[6] = Vector( -368, -3, 1 )
	local gearpos = {}
	gearpos[1] = Vector( 60, -92, -26 )
	gearpos[2] = Vector( 60, 92, -26 )
	gearpos[3] = Vector( -340, -3, 25 )
	...
	for i=1,2 do
		...
		self.Gear[i]:SetModel( "models/wwii/heinkel he-111_gear.mdl" )
		self.Gear[i]:SetAngles( self:GetAngles() + Angle(-90, 0, 0 ))
		self.Gear[i]:Spawn()
		self.Gear[i]:SetParent( self )
	
		self.Wheels[i] = ents.Create("prop_physics")
		self.Wheels[i]:SetPos( self:LocalToWorld( wheelpos[i] ) )
		self.Wheels[i]:SetModel( "models/wwii/heinkel he-111_wheel.mdl" )
	...
	local cname = {}
	cname[1] 	= "Roof Turret"
	cname[2] 	= "Belly Rear Turret"
	cname[3] 	= "Belly Front Turret"
	cname[4] 	= "Left Turret"
	cname[5] 	= "Right Turret"

	local cpos  = {}
	cpos[1] = Vector(  -64,2, 64 )
	cpos[2] = Vector( -133, 1, -17 )
	cpos[3] = Vector( -72, 2, -24 )
	cpos[4] = Vector( -108, 21, 36 )
	cpos[5] = Vector( -108, -21, 36 )

	-- Weapon Angle
	local cang  = {}
	cang[1] 	= Angle(-10,180,0)
	cang[2] 	= Angle(10,180,0)
	cang[3] 	= Angle(10,0,0)
	cang[4] 	= Angle(0,90,0)
	cang[5] 	= Angle(0,-90,0)
	...
	cmdl[1] = "models/fsx/warbirds/b-17 flying fortress_mgun.mdl"
	cmdl[2] = "models/fsx/warbirds/b-17 flying fortress_mgun.mdl"
	cmdl[3] = "models/fsx/warbirds/b-17 flying fortress_mgun.mdl"
	...
	self.PropellerPhys[i] = self.Propeller[i]:GetPhysicsObject()	
		if ( self.PropellerPhys[i]:IsValid() ) then
			...
			self.PropellerPhys[i]:SetMass( 5000 )
...
function ENT:TurretAttack( n )
	...
	local pos = self.Cannons[n]:LocalToWorld( Vector( 250, -16, 0 ) )
	
	if ( self.TurretSwitch ) then	
		
		pos = self.Cannons[n]:LocalToWorld( Vector( 250, 16, 0 ) )
...
function ENT:PhysicsSimulate( phys, deltatime )
	...
	if ( self.IsFlying && !stallang ) then
		...
		pr.pos 				= self:GetPos() + self:GetForward() * self.Speed + Vector( 0,0, -420 + ( self:GetVelocity():Length() / 3.5 ) )
...
--Propeller Script
	for i= 1, #self.Propeller do
		...
		if( IsValid( self.Propeller[i] ) ) then
			...
			self.Propeller[i]:GetPhysicsObject():AddAngleVelocity( Vector( self.Speed/100, 0, 0 ) )
