...
function ENT:CalcView( ply, Origin, Angles, Fov )
	...
	if( IsValid( plane ) && ply:GetNetworkedBool( "InFlight", false )  ) then
		...
		if( GetConVarNumber("jet_cockpitview") > 0 ) then
			...
			pos = plane:LocalToWorld( Vector( 153, 0, 124 ) ) //Origin//
...
function ENT:Draw()
	...
	self:DrawJetFlames( self:GetPos() + self:GetForward() * -306 + self:GetRight() * -24 + self:GetUp() * 89 )
	self:DrawJetFlames( self:GetPos() + self:GetForward() * -306 + self:GetRight() * 24 + self:GetUp() * 89 )
