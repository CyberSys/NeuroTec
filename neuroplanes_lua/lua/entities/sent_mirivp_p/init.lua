...
ENT.PrintName	= "Dassault Mirage IVP"
ENT.Model = "models/hawx/planes/dassault mirage ivp.mdl"
//Speed Limits
ENT.MaxVelocity = 1.8 * 2653
ENT.MinVelocity = 0
...
ENT.MachineGunOffset = Vector( 251, -2, 50 ) //No miniguns on Mirage IVP
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_mirivp_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	...
	*/
	...
	self.Armament = {
					...
					{ 
						PrintName = "AM-39 Exocet Short-Range Nuclear Missile"		,		 			// print name, used by the interface
						Mdl = "models/hawx/weapons/exocet am39.mdl",  		// model, used when creating the object
						Pos = Vector( -212, -167, 46 ), 							// Pos, Hard point location on the plane fuselage.
						...
						Class = "sent_a2s_nuclear_missile",								// the object that will be created.
					}; 
					{ 
						PrintName = "AM-39 Exocet Short-Range Nuclear Missile"		,	
						Mdl = "models/hawx/weapons/exocet am39.mdl",  		
						Pos = Vector( -212, 167, 46 ), 							
						Ang = Angle( 0, 0, 0), 								
						Type = "Homing", 										
						Cooldown = 20, 										
						isFirst	= false,										
						Class = "sent_a2s_nuclear_missile",					
					};

					{ 
						PrintName = "Drop Tank", 
						Mdl = "models/hawx/weapons/drop tank.mdl",	 
						Pos = Vector( -209 , -110, 52 ), 
						Ang = Angle( 0, 0, 0), 
						Type = "Effect",
						Cooldown = 0,
						isFirst	= false,
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -292, -223, 68 )
	self.TrailPos[2] = Vector( -292, 223, 68 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -338 + self:GetRight() * -23 + self:GetUp() * 89
		p[2] = self:GetPos() + self:GetForward() * -338 + self:GetRight() * 23 + self:GetUp() * 89
...
function ENT:PrimaryAttack()
	...
	if ( !IsValid( self.Pilot ) ) then
 	...
 	local bullet = {} 
 	bullet.Num 		= 1
 	bullet.Src 		= self.Weapon:GetPos() + self.Weapon:GetForward() * 155
