...
ENT.Model = "models/hawx/other_planes/pilotfish uav.mdl"
//Speed Limits
...
ENT.MachineGunOffset = Vector( 380, 0, -47 )
...
function ENT:PhysicsSimulate( phys, deltatime )
	...
	if ( IsValid( self.Pilot ) && IsValid( self.Target ) ) then
		...
		local apos = self:FindAvoidancePoint( self:GetPos(), Vector( 0, 0, 1 ), 3000 )
		
		if ( IsValid( self.PavePenny ) ) then
			
			if ( apos ) then // Skybox roof D:
				
				self.MaxAng = 250
				self.PavePenny:SetPos( apos )
			else
