...
ENT.PrintName	= "Su-27 Flanker"
ENT.Model = "models/hawx/planes/su-27 flanker.mdl"
//Speed Limits
...
ENT.MachineGunOffset = Vector( -29, 36, 103 )
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_su-27_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	...
	*/
	...
	self.Armament = {
					...
					{ 
						...
						Pos = Vector( -290, -281, 92), 							// Pos, Hard point location on the plane fuselage.
					...
					{ 
						PrintName = "Python-5"		,		 					// print name, used by the interface
						Mdl = "models/hawx/weapons/python-5.mdl",  				// model, used when creating the object
						Pos = Vector( -315, -236, 77), 							// Pos, Hard point location on the plane fuselage.
						Ang = Angle( 0, 0, 0), 									// Ang, object angle
						Type = "Homing", 										// Type, used when creating the object
						Cooldown = 3, 											// Cooldown between weapons
						--isFirst	= true,											// If a plane got 2 rockets of the same type, set the first rocket to isFirst = true.
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -382, -276, 92 )
	self.TrailPos[2] = Vector( -382, 276, 92 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -450 + self:GetRight() * -46 + self:GetUp() * 71
		p[2] = self:GetPos() + self:GetForward() * -450 + self:GetRight() * 46 + self:GetUp() * 71
		for _i=1,2 do
