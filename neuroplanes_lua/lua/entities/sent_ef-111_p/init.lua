...
ENT.PrintName	= "EF-111 Raven"
ENT.Model = "models/hawx/planes/ef-111 raven.mdl"
//Speed Limits
ENT.MaxVelocity = 1.8 * 2350
ENT.MinVelocity = 0
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_ef-111_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	...
	*/
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -157, -373, 105 )
	self.TrailPos[2] = Vector( -157, 373, 105 )
	...
	self:SpawnPassengerSeat( Vector( 188, -12, 63 ), Angle( 0, -90, 0 ) )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -335 + self:GetRight() * 36 + self:GetUp() * 84
		p[2] = self:GetPos() + self:GetForward() * -335 + self:GetRight() * -36 + self:GetUp() * 84
...
function ENT:Use(ply,caller)
	...
	if ( !self.IsFlying && !IsValid( self.Pilot ) ) then 
		...
		self.PilotModel = self:SpawnPilotModel( Vector( 187, 15, 73 ), Angle( 0, 0, 0 ) )
