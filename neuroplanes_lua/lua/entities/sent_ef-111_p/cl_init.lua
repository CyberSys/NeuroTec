...
function ENT:CalcView( ply, Origin, Angles, Fov )
	...
	if( IsValid( plane ) && ply:GetNetworkedBool( "InFlight", false )  ) then
		...
		if( GetConVarNumber("jet_cockpitview") > 0 ) then
			...
			pos = plane:LocalToWorld( Vector( 191, 14, 104 ) ) //Origin//
			
			if( IsValid( pilotmodel ) ) then
				
				pilotmodel:SetNoDraw( true )
...
function ENT:Draw()
	...
	if( self:GetNetworkedInt( "health" , 0 ) > 100 ) then
		...
		self:DrawJetFlames( self:GetPos() + self:GetForward() * -335 + self:GetRight() * 36 + self:GetUp() * 84 )
		self:DrawJetFlames( self:GetPos() + self:GetForward() * -335 + self:GetRight() * -36 + self:GetUp() * 84 )
