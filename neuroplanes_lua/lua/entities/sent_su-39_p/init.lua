...
ENT.PrintName	= "Su-39 Frogfoot"
ENT.Model = "models/hawx/planes/su-39 frogfoot.mdl"
//Speed Limits
ENT.MaxVelocity = 1.8 * 1050
ENT.MinVelocity = 0
...
ENT.MachineGunOffset = Vector( 196, -10, 50 )
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_su-39_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	...
	*/
	...
	self.Armament = {
					...
					{ 
						...
						Pos = Vector( -49, -216, 77), 							// Pos, Hard point location on the plane fuselage.
					...
					{ 
						...
						Pos = Vector( -52, -181, 74), 							// Pos, Hard point location on the plane fuselage.
						Ang = Angle( 0, 0, 0), 								// Ang, object angle
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -90, -286, 94 )
	self.TrailPos[2] = Vector( -90, 286, 94 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -119 + self:GetRight() * -36 + self:GetUp() * 70
		p[2] = self:GetPos() + self:GetForward() * -119 + self:GetRight() * 36 + self:GetUp() * 70
		for _i=1,2 do
...
function ENT:PrimaryAttack()
	...
	if ( !IsValid( self.Pilot ) ) then
 	...
 	local bullet = {} 
 	bullet.Num 		= 1
 	bullet.Src 		= self.Weapon:GetPos() + self.Weapon:GetForward() * 100
