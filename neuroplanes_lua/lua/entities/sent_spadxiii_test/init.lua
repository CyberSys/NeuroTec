...
ENT.Model = "models/inaki/props_vehicles/spad_xiii.mdl"
//Speed Limits
ENT.MaxVelocity = 1.8 * 500
ENT.MinVelocity = 0
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_spadxiii_test" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
self.NumRockets = #self.EquipmentNames or 0
	...
	local wheelpos = {}
	wheelpos[1] = Vector( 25, -55, -75 )
	wheelpos[2] = Vector( 25, 55, -75 )
	...
	for i = 1, 2 do
		...
		self.Wheels[i]:SetModel( "models/inaki/props_vehicles/spad_xiii_wheel.mdl" )
...
//END ADDED SMOKE EFFECT
 ...
 local minipos = { Vector( 41.4, 11.2, 28.56), Vector( 41.4, -11.2, 28.56)}
	self.Miniguns = {}
...
function ENT:Use(ply,caller)
	...
	if ( !self.IsFlying && !IsValid( self.Pilot ) ) then 
		...
		self.pilotmodel:SetPos( self:GetPos() + self:GetRight() * 0 + self:GetForward() * -48 + self:GetUp() * 8 )
