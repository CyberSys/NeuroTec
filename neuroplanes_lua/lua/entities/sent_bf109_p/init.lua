...
ENT.PrintName = "Messerschmitt BF109"
ENT.Model = "models/fsx/warbirds/bf109.mdl"
--Speed Limits
...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_bf109_p" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	...
	self.Armament = {
					...
					-- { 
						...
						-- Col = Color( 0, 0, 0, 0 ), 							
						-- Pos = Vector( 91, 75, -10), 							-- Pos, Hard point location on the plane fuselage.
					...
					-- { 
						...
						-- Mdl = "models/items/AR2_grenade.mdl" ,
						-- Col = Color( 0, 0, 0, 0 ), 							
						-- Pos = Vector( 89, -76.5, -10), 							-- Pos, Hard point location on the plane fuselage.
						-- Ang = Angle( 0, 0, 0), 								-- Ang, object angle
						-- Type = "Dumb", 										-- Type, used when creating the object
						-- Cooldown = 0.05, 										-- Cooldown between weapons
					...
					{ 
						...
						Pos = Vector( 20, 0, -5), 							-- Pos, Hard point location on the plane fuselage.
						Ang = Angle( 0, 0, 0), 								-- Ang, object angle
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( 28.5, 263, 31 )
	self.TrailPos[2] = Vector( 28.5, -253, 31 )
	
	-- 28.6274 -263.0553 31.8179
	...
	self.IsFlying = false
	self.Pilot = NULL
	-- 65.0130 -6.6143 51.3838

	local minipos = { Vector( 55, 6, 51 ), Vector( 55, -6, 51 ) }
	self.Miniguns = {}
	...
	local PropellerPos = Vector( 123, 0, 30)
	self.Propeller = ents.Create("prop_physics")
	self.Propeller:SetPos( self:LocalToWorld( PropellerPos ) )
	self.Propeller:SetModel( "models/fsx/warbirds/bf109rotor.mdl" )
...
-- --	wheelpos[2] = Vector( 0, 0, 0 )
	wheelpos[1] = Vector( 45, 45, -25 )
	wheelpos[2] = Vector( 45, -45, -25 )
	wheelpos[3] = Vector( 45, 45, -25 )
	wheelpos[4] = Vector( 45, -45, -25 )
	local gearpos = {}
	gearpos[1] = Vector( 52, 47, 15 )
	gearpos[2] = Vector( 52, -47, 15 )
...
function ENT:PhysicsSimulate( phys, deltatime )
	...
	if ( self.IsFlying && !stallang ) then
		...
		local downforce = Vector( 0,0, -300 + ( self:GetVelocity():Length() / 4 ) )
		-- if( self.Speed < 100 ) then downforce = Vector( 0,0,0 )  end
		
		local pr = {}
		pr.secondstoarrive	= 1
		pr.pos 				= self:GetPos() + self:GetForward() * self.Speed + downforce
