...
function ENT:SpawnFunction( ply, tr)
	...
	local ent = ents.Create( "sent_f16fightingfalcon" )
	ent:SetPos( SpawnPos )
	ent:Spawn()
	ent:Activate()
	...
	*/
	...
	self.Armament = {
					...
					{ 
						...
						Pos = Vector( -54, -157, -14), 							// Pos, Hard point location on the plane fuselage.
						Ang = Angle( 0, 0, 45), 								// Ang, object angle
	...
	self.Trails = {}
	self.TrailPos = {}
	self.TrailPos[1] = Vector( -87, -183, 3 )
	self.TrailPos[2] = Vector( -87, 183, 3 )
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < self.InitialHealth * 0.15 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -168 + self:GetRight() * 0 + self:GetUp() * 2
		p[2] = self:GetPos() + self:GetForward() * -168 + self:GetRight() * 0 + self:GetUp() * -2
		for _i=1,2 do
...
function ENT:PhysicsSimulate( phys, deltatime )
	...
	if ( self.IsFlying ) then
		...
		vel = math.Clamp( vel, 0, 120 )
		
		local pr = {}
		pr.secondstoarrive	= 1
		pr.pos 				= self:GetPos() + self:GetForward() * self.Speed + Vector(0,0,-100+vel )
