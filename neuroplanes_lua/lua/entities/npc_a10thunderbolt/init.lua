...
ENT.MachineGunOffset = Vector( 255, 0, 67 )
...
function ENT:Initialize()
	...
	self.RocketVisuals = {}
	self.Armament = {
					{ Mdl = "models/hawx/weapons/lau-131 a.mdl", Pos = Vector( -10, -230, 75), Ang = Angle( 0, 0, 0) }; // Rocket Pod
					{ Mdl = "models/hawx/weapons/lau-131 a.mdl", Pos = Vector( -10, 230, 75), Ang = Angle( 0, 0, 0) }; // Rocket Pod
					{ Mdl = "models/military2/missile/missile_agm88.mdl", Pos = Vector( -10, -140, 70 ), Ang = Angle( 0, 0, 45 ) }; // Right AGM88 Rocket
					{ Mdl = "models/military2/missile/missile_agm88.mdl", Pos = Vector( -10, 140, 70 ), Ang = Angle( 0, 0, 45 ) }; // Left AGM88 Rocket
					}
...
function ENT:OnTakeDamage(dmginfo)
	...
	if ( self.HealthVal < 200 && !self.Burning ) then
		...
		self.Burning = true
		local p = {}
		p[1] = self:GetPos() + self:GetForward() * -200 + self:GetRight() * -56 + self:GetUp() * 120 
		p[2] = self:GetPos() + self:GetForward() * -200 + self:GetRight() * 56 + self:GetUp() * 120 
		for _i=1,2 do
...
function ENT:IsThisSafeAirspace( pos, radius ) 
	
	local hitcount = 0
	
	for i=1,36 do
		
		local rand = math.random( -radius, radius )
		
		local tr, trace = {},{}
		tr.start = pos + Vector( rand, rand, 5000 )
		tr.endpos = pos + Vector( -rand, -rand, 5000 )
		tr.mask = MASK_SOLID
		tr.filter = { self }
		trace = util.TraceLine( tr )
		
		debugoverlay.Line( tr.start, trace.HitPos, 15, Color( 255, 0, 0, 0 ), true )
...
function ENT:Think()
	...
	if( IsValid( self.Target ) ) then
		...
		if( self.Target:OnGround() && logic1 && logic2 ) then // Fly in, shoot, leave.
			...
			if( self:IsThisSafeAirspace( self.Target:GetPos(), 4000 ) ) then
				
				self.Target = self.PavePenny 
			...
			for i=1,2 do 
				...
				timer.Simple( 1/i, function() if( IsValid( self ) ) then
		...
		elseif( self.Target:OnGround() && !logic1 || trace.HitWorld ) then
			...
			local p = self.Target:GetPos() + self.Target:GetUp() * 5048
		...
		elseif ( !self.Target:OnGround() ) then
			...
			self.PavePenny:SetPos( self.Target:GetPos() + self.Target:GetForward() * 256 + self.Target:GetUp() * 2500 )
...
function ENT:PrimaryAttack()
 	...
 	bullet.Spread 	= Vector( .012, .012, .012 )				// Aim Cone 
 	...
 	bullet.Callback    = function ( a, b, c )
							...
							util.BlastDamage( self.Pilot, self.Pilot, b.HitPos, 150, 18 )
	...
	local sm = EffectData()
	sm:SetStart( self.Weapon:GetPos() )
	sm:SetOrigin( self.Weapon:GetPos() )
	sm:SetScale( 2.5 )
	util.Effect( "a10_muzzlesmoke", sm )
	ParticleEffect( "apc_muzzleflash", self.Weapon:GetPos() + self.Weapon:GetForward() * 55,  self.Weapon:GetAngles(), self.Weapon )
...
function ENT:FindAvoidancePoint( pos, dir, length )
	...
	if ( newPos ) then
		...
		debugoverlay.Line( newPos, trace.HitPos, 0.1, Color( 255, 0, 0, 0 ), true )
...
function ENT:PhysicsSimulate( phys, deltatime )
	...
	if ( IsValid( self.Pilot ) && IsValid( self.Target ) ) then
		...
		local fpos = self:FindAvoidancePoint( self:GetPos(), self:GetForward(), 12000 )
		...
		if ( IsValid( self.PavePenny ) ) then
			...
			if ( fpos ) then
				...
				debugoverlay.Line( fpos, self:GetPos(), 0.5, Color( 255, 0, 0, 0 ), true )
				
				-- self:DrawLaserTracer( self:GetPos() + self:GetForward() * 10000, fpos )
			end
